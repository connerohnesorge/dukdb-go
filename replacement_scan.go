...
func NewReplacementScanContext(
	ctx context.Context,
	clock quartz.Clock,
) *ReplacementScanContext {
	if clock == nil {
...
func (c *ReplacementScanContext) WithClock(
	clock quartz.Clock,
) *ReplacementScanContext {
	return &ReplacementScanContext{
		ctx:   c.ctx,
...
func (r *replacementScanRegistry) register(
	callback ReplacementScanCallback,
) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func validateReplacementParams(
	params []any,
) error {
...
func TryReplacementScan(
	c *Connector,
	tableName string,
) ReplacementScanResult {
	return TryReplacementScanWithContext(
		c,
		tableName,
		nil,
	)
...
func TryReplacementScanWithContext(
	c *Connector,
	tableName string,
	ctx *ReplacementScanContext,
) ReplacementScanResult {
...
func ConvertParamsToFunctionArgs(
	params []any,
) []FunctionArg {
	if len(params) == 0 {
		return nil
