...
func TestDataChunkValueAccess(t *testing.T) {
	...
	t.Run(
		"get value invalid column index",
		func(t *testing.T) {
			intType, err := NewTypeInfo(
				TYPE_INTEGER,
			)
	...
	t.Run(
		"get value negative column index",
		func(t *testing.T) {
			intType, err := NewTypeInfo(
				TYPE_INTEGER,
			)
...
func TestSetChunkValueGeneric(t *testing.T) {
	t.Run(
		"set chunk value generic",
		func(t *testing.T) {
			doubleType, err := NewTypeInfo(
				TYPE_DOUBLE,
			)
			require.NoError(t, err)
	...
	t.Run(
		"set chunk value generic type mismatch",
		func(t *testing.T) {
			intType, err := NewTypeInfo(
				TYPE_INTEGER,
			)
...
func TestColumnProjection(t *testing.T) {
	t.Run(
		"unprojected column ignored on set",
		func(t *testing.T) {
			intType, err := NewTypeInfo(
				TYPE_INTEGER,
			)
...
func TestVectorPrimitiveTypes(t *testing.T) {
	t.Run("boolean vector", func(t *testing.T) {
		...
		require.NoError(
			t,
			chunk.SetValue(0, 1, false),
		)
		require.NoError(
			t,
			chunk.SetValue(0, 2, nil),
		)
			...
			for _, tt := range tests {
				t.Run(
					...
					func(t *testing.T) {
						...
						gotMin, _ := chunk.GetValue(
							0,
							0,
						)
						gotMax, _ := chunk.GetValue(
							0,
							1,
						)
...
func TestVectorStringTypes(t *testing.T) {
	t.Run("VARCHAR vector", func(t *testing.T) {
		...
		require.NoError(
			t,
			chunk.SetValue(0, 0, "hello world"),
		)
		require.NoError(
			t,
			chunk.SetValue(0, 1, ""),
...
func TestVectorTemporalTypes(t *testing.T) {
	t.Run(
		"TIMESTAMP vector precision variants",
		func(t *testing.T) {
			now := time.Date(
				2024,
				1,
...
func TestVectorComplexNumericTypes(t *testing.T) {
	t.Run("HUGEINT vector", func(t *testing.T) {
		...
		largeVal := new(
			big.Int,
		).SetUint64(math.MaxUint64)
		largeVal = largeVal.Add(
			largeVal,
			big.NewInt(1),
		)

		hugeintType, err := NewTypeInfo(
			TYPE_HUGEINT,
		)
		require.NoError(t, err)

		chunk, err := NewDataChunk(
			[]TypeInfo{hugeintType},
		)
		require.NoError(t, err)
...
func TestVectorEnumType(t *testing.T) {
	t.Run(
		"ENUM vector set by name",
		func(t *testing.T) {
			enumType, err := NewEnumInfo(
				"red",
				"green",
				"blue",
			)
...
func TestDataChunkInitialization(t *testing.T) {
	t.Run(
		"initialize from types",
		func(t *testing.T) {
			intType, _ := NewTypeInfo(
				TYPE_INTEGER,
			)
			varcharType, _ := NewTypeInfo(
				TYPE_VARCHAR,
			)
			boolType, _ := NewTypeInfo(
				TYPE_BOOLEAN,
			)
...
func TestDataChunkReset(t *testing.T) {
	t.Run(
		"reset preserves column structure",
		func(t *testing.T) {
			intType, _ := NewTypeInfo(
				TYPE_INTEGER,
			)
...
func TestDataChunkUhugeint(t *testing.T) {
	t.Run(
		"basic uhugeint operations",
		func(t *testing.T) {
			uhugeintType, err := NewTypeInfo(
				TYPE_UHUGEINT,
			)
			require.NoError(t, err)

			chunk, err := NewDataChunk(
				[]TypeInfo{uhugeintType},
			)
			require.NoError(t, err)
...
func BenchmarkDataChunk_ScanThroughput(
	b *testing.B,
) {
	...
	b.ReportMetric(
		float64(GetDataChunkCapacity()),
		"rows/op",
	)
}

func BenchmarkDataChunk_ScanMultiColumn(
	b *testing.B,
) {
	intType, _ := NewTypeInfo(TYPE_INTEGER)
	...
	b.ReportMetric(
		float64(GetDataChunkCapacity()*3),
		"values/op",
	)
}
