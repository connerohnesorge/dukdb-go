...
func TestDataChunkToRecordBatch_Primitives(
	t *testing.T,
) {
	t.Parallel()
	...
	require.NoError(
		t,
		chunk.SetValue(1, 0, "hello"),
	)
	require.NoError(t, chunk.SetValue(2, 0, true))

	require.NoError(
		t,
		chunk.SetValue(0, 1, int32(123)),
	)
	require.NoError(
		t,
		chunk.SetValue(1, 1, "world"),
	)
	require.NoError(
		t,
		chunk.SetValue(2, 1, false),
	)
	...
	record, err := DataChunkToRecordBatch(
		chunk,
		schema,
		alloc,
	)
...
func TestDataChunkToRecordBatch_WithNulls(
	t *testing.T,
) {
	t.Parallel()
	...
	require.NoError(
		t,
		chunk.SetValue(0, 1, nil),
	) // NULL
	require.NoError(
		t,
		chunk.SetValue(0, 2, int32(99)),
	)
...
func TestDataChunkToRecordBatch_AllNumericTypes(
	t *testing.T,
) {
	t.Parallel()
...
func TestDataChunkToRecordBatch_Timestamps(
	t *testing.T,
) {
	t.Parallel()
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			case arrow.Second:
				got = time.Unix(int64(tsVal), 0).
					UTC()
			case arrow.Millisecond:
				got = time.UnixMilli(int64(tsVal)).
					UTC()
			case arrow.Microsecond:
				got = time.UnixMicro(int64(tsVal)).
					UTC()
			case arrow.Nanosecond:
				got = time.Unix(0, int64(tsVal)).
					UTC()
			}
...
func TestRecordBatchToDataChunk_Primitives(
	t *testing.T,
) {
	t.Parallel()
	...
	chunk, err := NewDataChunk(
		[]TypeInfo{intInfo, strInfo, floatInfo},
	)
	require.NoError(t, err)
...
func TestRecordBatchToDataChunk_WithNulls(
	t *testing.T,
) {
	t.Parallel()
...
func TestRoundTrip_DataChunk_RecordBatch(
	t *testing.T,
) {
	t.Parallel()
	...
	roundtrip, err := NewDataChunk(
		[]TypeInfo{intInfo, strInfo, boolInfo},
	)
	require.NoError(t, err)

	err = RecordBatchToDataChunk(
		record,
		roundtrip,
	)
	require.NoError(t, err)
	...
	for row := range 3 {
		for col := range 3 {
			origVal, err := original.GetValue(
				col,
				row,
			)
			require.NoError(t, err)
			rtVal, err := roundtrip.GetValue(
				col,
				row,
			)
			require.NoError(t, err)
...
func TestTypesFromSchema(t *testing.T) {
	...
	assert.Equal(
		t,
		TYPE_BOOLEAN,
		types[2].InternalType(),
	)
}

func TestDataChunkToRecordBatch_List(
	t *testing.T,
) {
	t.Parallel()
...
func TestDataChunkToRecordBatch_ErrorCases(
	t *testing.T,
) {
	t.Parallel()
...
func TestRecordBatchToDataChunk_ErrorCases(
	t *testing.T,
) {
	t.Parallel()
	...
	t.Run(
		"column count mismatch",
		func(t *testing.T) {
			intInfo2, _ := NewTypeInfo(
				TYPE_INTEGER,
			)
			chunk2, _ := NewDataChunk(
				[]TypeInfo{intInfo2, intInfo2},
			)
			err := RecordBatchToDataChunk(
				record,
				chunk2,
...
func TestDataChunkToRecordBatch_UUID(
	t *testing.T,
) {
	t.Parallel()
	...
	schema := arrow.NewSchema([]arrow.Field{
		{
			Name: "uuid",
			Type: &arrow.FixedSizeBinaryType{
				ByteWidth: 16,
			},
			Nullable: true,
		},
...
func TestDataChunkToRecordBatch_Decimal(
	t *testing.T,
) {
	t.Parallel()
...
func TestArrowTypeToDuckDBRoundTrip(
	t *testing.T,
) {
	t.Parallel()
