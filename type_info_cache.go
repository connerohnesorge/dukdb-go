...
func getCachedPrimitiveTypeInfo(
	t Type,
) (TypeInfo, error) {
	...
	case TYPE_INVALID, TYPE_BOOLEAN, TYPE_TINYINT, TYPE_SMALLINT, TYPE_INTEGER,
		...
		// These are primitive types that can be cached - continue below
	}

	// Store and return (LoadOrStore handles race conditions)
	actual, _ := primitiveTypeInfoCache.LoadOrStore(
		t,
		info,
	)
