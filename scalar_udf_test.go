...
func createTestVector(typeInfo TypeInfo, capacity int) (*vector, error) {
...
func TestScalarFuncRegistry(t *testing.T) {
	t.Run("register and lookup basic function", func(t *testing.T) {
		...
		err = registry.register("test_func", fn)
		require.NoError(t, err)

		result := registry.lookup("test_func", []Type{TYPE_INTEGER})
		require.NotNil(t, result)
		assert.Equal(t, "test_func", result.name)
	})

	t.Run("lookup non-existent function", func(t *testing.T) {
		registry := newScalarFuncRegistry()
		result := registry.lookup("non_existent", []Type{TYPE_INTEGER})
		assert.Nil(t, result)
	})
	...
	t.Run("register nil function", func(t *testing.T) {
		registry := newScalarFuncRegistry()
		err := registry.register("test_func", nil)
		require.Error(t, err)
		assert.Contains(t, err.Error(), "cannot be nil")
	})

	t.Run("register function with nil result type", func(t *testing.T) {
		registry := newScalarFuncRegistry()
	...
	t.Run("register function with TYPE_ANY result", func(t *testing.T) {
		registry := newScalarFuncRegistry()
		...
		err := registry.register("test_func", fn)
		require.Error(t, err)
		assert.Contains(t, err.Error(), "cannot be TYPE_ANY")
	})

	t.Run("register function without executor", func(t *testing.T) {
		registry := newScalarFuncRegistry()
		...
		err = registry.register("test_func", fn)
		require.Error(t, err)
		assert.Contains(t, err.Error(), "must have either RowExecutor or RowContextExecutor")
	})
}
...
func TestScalarFuncOverloading(t *testing.T) {
	t.Run("register multiple overloads", func(t *testing.T) {
		...
		err = registry.register("overloaded", fn1)
		require.NoError(t, err)

		err = registry.register("overloaded", fn2)
		require.NoError(t, err)

		// Lookup with INTEGER type
		result := registry.lookup("overloaded", []Type{TYPE_INTEGER})
		require.NotNil(t, result)

		// Lookup with VARCHAR type
		result = registry.lookup("overloaded", []Type{TYPE_VARCHAR})
		require.NotNil(t, result)
	})

	t.Run("lookup with non-matching type", func(t *testing.T) {
		registry := newScalarFuncRegistry()
		...
		result := registry.lookup("test_func", []Type{TYPE_VARCHAR})
		assert.Nil(t, result)
	})
}
...
func TestTypeMatching(t *testing.T) {
	t.Run("exact type match", func(t *testing.T) {
		assert.True(t, typesCompatible(TYPE_INTEGER, TYPE_INTEGER))
		assert.True(t, typesCompatible(TYPE_VARCHAR, TYPE_VARCHAR))
		assert.False(t, typesCompatible(TYPE_INTEGER, TYPE_VARCHAR))
	})

	t.Run("TYPE_ANY matches anything", func(t *testing.T) {
		assert.True(t, typesCompatible(TYPE_ANY, TYPE_INTEGER))
		assert.True(t, typesCompatible(TYPE_ANY, TYPE_VARCHAR))
		assert.True(t, typesCompatible(TYPE_ANY, TYPE_BOOLEAN))
	})

	t.Run("variadic parameter matching", func(t *testing.T) {
		intType, err := NewTypeInfo(TYPE_INTEGER)
		require.NoError(t, err)
		...
		assert.True(t, f.matchesTypes([]Type{TYPE_INTEGER}))

		// Should match with multiple arguments
		assert.True(t, f.matchesTypes([]Type{TYPE_INTEGER, TYPE_INTEGER, TYPE_INTEGER}))

		// Should not match with wrong type
		assert.False(t, f.matchesTypes([]Type{TYPE_VARCHAR}))
	})

	t.Run("variadic with TYPE_ANY", func(t *testing.T) {
		intType, err := NewTypeInfo(TYPE_INTEGER)
		require.NoError(t, err)
		anyType := &typeInfo{typ: TYPE_ANY}
		...
		assert.True(t, f.matchesTypes([]Type{TYPE_INTEGER, TYPE_VARCHAR, TYPE_BOOLEAN}))
	})

	t.Run("non-variadic arg count mismatch", func(t *testing.T) {
		intType, err := NewTypeInfo(TYPE_INTEGER)
		require.NoError(t, err)

		f := &registeredScalarFunc{
			config: ScalarFuncConfig{
				InputTypeInfos: []TypeInfo{intType, intType},
				ResultTypeInfo: intType,
			},
		}

		// Too few args
		assert.False(t, f.matchesTypes([]Type{TYPE_INTEGER}))

		// Too many args
		assert.False(t, f.matchesTypes([]Type{TYPE_INTEGER, TYPE_INTEGER, TYPE_INTEGER}))

		// Correct count
		assert.True(t, f.matchesTypes([]Type{TYPE_INTEGER, TYPE_INTEGER}))
	})
}

// TestScalarFuncExecution tests scalar function execution.
func TestScalarFuncExecution(t *testing.T) {
	t.Run("execute basic function", func(t *testing.T) {
		intType, err := NewTypeInfo(TYPE_INTEGER)
		require.NoError(t, err)
		...
		ctx := NewScalarFuncContext(context.Background(), nil)
		err = executeScalarUDF(ctx, udf, chunk, output)
		require.NoError(t, err)
	...
	t.Run("null handling - null in null out", func(t *testing.T) {
		intType, err := NewTypeInfo(TYPE_INTEGER)
		require.NoError(t, err)
		...
		err = chunk.SetValue(0, 0, nil) // NULL
		require.NoError(t, err)
	...
	t.Run("special null handling", func(t *testing.T) {
		intType, err := NewTypeInfo(TYPE_INTEGER)
		require.NoError(t, err)
	...
	t.Run("panic recovery", func(t *testing.T) {
		...
		err = executeScalarUDF(ctx, udf, chunk, output)
		require.Error(t, err)
		assert.Contains(t, err.Error(), "panic in scalar UDF")
	})

	t.Run("user error propagation", func(t *testing.T) {
		intType, err := NewTypeInfo(TYPE_INTEGER)
		require.NoError(t, err)

		expectedErr := errors.New("user defined error")
		udf := &registeredScalarFunc{
			name: "error_func",
			config: ScalarFuncConfig{
				InputTypeInfos: []TypeInfo{intType},
		...
		err = executeScalarUDF(ctx, udf, chunk, output)
		require.Error(t, err)
		assert.Contains(t, err.Error(), "user defined error")
	})
}

// TestScalarFuncContext tests the ScalarFuncContext.
func TestScalarFuncContext(t *testing.T) {
	t.Run("create with nil clock", func(t *testing.T) {
		ctx := NewScalarFuncContext(context.Background(), nil)
	...
	t.Run("create with mock clock", func(t *testing.T) {
		mockClock := quartz.NewMock(t)
		ctx := NewScalarFuncContext(context.Background(), mockClock)
		require.NotNil(t, ctx)
		assert.Equal(t, mockClock, ctx.Clock())
	})

	t.Run("WithClock creates new context", func(t *testing.T) {
		mockClock1 := quartz.NewMock(t)
		mockClock2 := quartz.NewMock(t)

		baseCtx := context.Background()
		ctx1 := NewScalarFuncContext(baseCtx, mockClock1)
		ctx2 := ctx1.WithClock(mockClock2)

		assert.Equal(t, mockClock1, ctx1.Clock())
		assert.Equal(t, mockClock2, ctx2.Clock())
...
func TestDeterministicTimeout(t *testing.T) {
	t.Run("timeout not exceeded", func(t *testing.T) {
		...
		deadline := baseTime.Add(1 * time.Second)
		goCtx, cancel := context.WithDeadline(context.Background(), deadline)
		defer cancel()

		ctx := NewScalarFuncContext(goCtx, mockClock)
	...
	t.Run("timeout exceeded", func(t *testing.T) {
		...
		deadline := baseTime.Add(-1 * time.Second) // Already passed
		goCtx, cancel := context.WithDeadline(context.Background(), deadline)
		defer cancel()
	...
	t.Run("timeout during execution - deterministic", func(t *testing.T) {
		mockClock := quartz.NewMock(t)
		baseTime := time.Date(2024, 6, 15, 14, 0, 0, 0, time.UTC)
		mockClock.Set(baseTime)

		deadline := baseTime.Add(100 * time.Millisecond)
		goCtx, cancel := context.WithDeadline(context.Background(), deadline)
		defer cancel()
	...
	t.Run("no timeout without deadline", func(t *testing.T) {
		mockClock := quartz.NewMock(t)
		baseTime := time.Date(2024, 6, 15, 14, 0, 0, 0, time.UTC)
		mockClock.Set(baseTime)
...
func TestRowContextExecutor(t *testing.T) {
	t.Run("context executor receives context", func(t *testing.T) {
		intType, err := NewTypeInfo(TYPE_INTEGER)
		require.NoError(t, err)
		...
		udf := &registeredScalarFunc{
			...
			bindCtx: context.WithValue(context.Background(), ctxKey("key"), "value"),
		}
...
func TestScalarFuncPrimitiveTypes(t *testing.T) {
	testCases := []struct {
		...
		{"TINYINT", TYPE_TINYINT, int8(42), int8(42)},
		{"SMALLINT", TYPE_SMALLINT, int16(1000), int16(1000)},
		{"INTEGER", TYPE_INTEGER, int32(100000), int32(100000)},
		{"BIGINT", TYPE_BIGINT, int64(1000000000), int64(1000000000)},
		{"FLOAT", TYPE_FLOAT, float32(3.14), float32(3.14)},
		{"DOUBLE", TYPE_DOUBLE, float64(3.14159265), float64(3.14159265)},
		{"VARCHAR", TYPE_VARCHAR, "hello", "hello"},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			udf := &registeredScalarFunc{
				name: "identity",
				config: ScalarFuncConfig{
					InputTypeInfos: []TypeInfo{typeInfo},
...
func TestScalarFuncMultipleInputs(t *testing.T) {
	t.Run("add two integers", func(t *testing.T) {
		...
		chunk, err := NewDataChunk([]TypeInfo{intType, intType})
		require.NoError(t, err)
	...
	t.Run("concat two strings", func(t *testing.T) {
		varcharType, err := NewTypeInfo(TYPE_VARCHAR)
		require.NoError(t, err)
		...
		udf := &registeredScalarFunc{
			name: "concat",
			config: ScalarFuncConfig{
				InputTypeInfos: []TypeInfo{varcharType, varcharType},
		...
		chunk, err := NewDataChunk([]TypeInfo{varcharType, varcharType})
		require.NoError(t, err)
...
func TestVolatileFunction(t *testing.T) {
	t.Run("volatile function config", func(t *testing.T) {
		intType, err := NewTypeInfo(TYPE_INTEGER)
		require.NoError(t, err)
...
func TestScalarFuncRegistryLookupScalarUDF(t *testing.T) {
	t.Run("lookup existing function returns UDF info", func(t *testing.T) {
		registry := newScalarFuncRegistry()
		...
		udfInfo, resultType, found := registry.LookupScalarUDF("my_func", []Type{TYPE_INTEGER})
		require.True(t, found)
		assert.NotNil(t, udfInfo)
		assert.Equal(t, TYPE_INTEGER, resultType)
	})

	t.Run("lookup non-existent function returns not found", func(t *testing.T) {
		registry := newScalarFuncRegistry()

		udfInfo, resultType, found := registry.LookupScalarUDF("non_existent", []Type{TYPE_INTEGER})
		assert.False(t, found)
		assert.Nil(t, udfInfo)
		assert.Equal(t, TYPE_INVALID, resultType)
	})

	t.Run("lookup with wrong types returns not found", func(t *testing.T) {
		registry := newScalarFuncRegistry()
		...
		err = registry.register("typed_func", fn)
		require.NoError(t, err)

		// Try to lookup with wrong type
		udfInfo, resultType, found := registry.LookupScalarUDF("typed_func", []Type{TYPE_VARCHAR})
		assert.False(t, found)
		assert.Nil(t, udfInfo)
		assert.Equal(t, TYPE_INVALID, resultType)
	})
...
func TestScalarFuncComplexTypes(t *testing.T) {
	t.Run("LIST input and output", func(t *testing.T) {
		registry := newScalarFuncRegistry()
		...
		err = registry.register("list_identity", fn)
		require.NoError(t, err)

		result := registry.lookup("list_identity", []Type{TYPE_LIST})
		require.NotNil(t, result)
	})

	t.Run("STRUCT input and output", func(t *testing.T) {
		registry := newScalarFuncRegistry()

		intType, err := NewTypeInfo(TYPE_INTEGER)
		require.NoError(t, err)

		strType, err := NewTypeInfo(TYPE_VARCHAR)
		require.NoError(t, err)

		idEntry, err := NewStructEntry(intType, "id")
		require.NoError(t, err)

		nameEntry, err := NewStructEntry(strType, "name")
		require.NoError(t, err)

		structType, err := NewStructInfo(idEntry, nameEntry)
		require.NoError(t, err)
		...
		err = registry.register("struct_identity", fn)
		require.NoError(t, err)

		result := registry.lookup("struct_identity", []Type{TYPE_STRUCT})
		require.NotNil(t, result)
	})

	t.Run("MAP input and output", func(t *testing.T) {
		registry := newScalarFuncRegistry()
		...
		mapType, err := NewMapInfo(strType, intType)
		require.NoError(t, err)
		...
		err = registry.register("map_identity", fn)
		require.NoError(t, err)

		result := registry.lookup("map_identity", []Type{TYPE_MAP})
		require.NotNil(t, result)
	})
}
...
func TestScalarFuncConstantFolding(t *testing.T) {
	t.Run("BindScalarUDF with no ScalarBinder returns nil", func(t *testing.T) {
		registry := newScalarFuncRegistry()
		...
		err = registry.register("identity", fn)
		require.NoError(t, err)

		udfInfo, _, found := registry.LookupScalarUDF("identity", []Type{TYPE_INTEGER})
		require.True(t, found)
		...
		bindCtx, err := registry.BindScalarUDF(udfInfo, args)
		require.NoError(t, err)
		require.Nil(t, bindCtx) // No ScalarBinder defined
	})

	t.Run("BindScalarUDF with ScalarBinder is called", func(t *testing.T) {
		registry := newScalarFuncRegistry()
		...
		err = registry.register("with_binder", fn)
		require.NoError(t, err)

		udfInfo, _, found := registry.LookupScalarUDF("with_binder", []Type{TYPE_INTEGER, TYPE_INTEGER})
		require.True(t, found)
		...
		require.True(t, receivedArgs[0].Foldable)
		require.Equal(t, int32(42), receivedArgs[0].Value)
		require.False(t, receivedArgs[1].Foldable)
		require.NotNil(t, bindCtx)
	})

	t.Run("BindScalarUDF error propagation", func(t *testing.T) {
		registry := newScalarFuncRegistry()
		...
		err = registry.register("failing_binder", fn)
		require.NoError(t, err)

		udfInfo, _, found := registry.LookupScalarUDF("failing_binder", []Type{TYPE_INTEGER})
		require.True(t, found)

		args := []ScalarUDFArg{{Foldable: true, Value: int32(1)}}

		_, err = registry.BindScalarUDF(udfInfo, args)
		require.Error(t, err)
		require.Contains(t, err.Error(), "bind failed")
	})

	t.Run("BindScalarUDF with invalid udfInfo returns error", func(t *testing.T) {
		registry := newScalarFuncRegistry()

		args := []ScalarUDFArg{{Foldable: true, Value: int32(1)}}

		_, err := registry.BindScalarUDF("not a udf", args)
		require.Error(t, err)
		require.Contains(t, err.Error(), "invalid UDF info type")
	})
}
...
func TestScalarFuncVolatileCaching(t *testing.T) {
	t.Run("IsVolatile returns true for volatile function", func(t *testing.T) {
		registry := newScalarFuncRegistry()
		...
		err = registry.register("volatile_fn", fn)
		require.NoError(t, err)

		udfInfo, _, found := registry.LookupScalarUDF("volatile_fn", []Type{})
		require.True(t, found)

		isVolatile := registry.IsVolatile(udfInfo)
		require.True(t, isVolatile)
	})

	t.Run("IsVolatile returns false for non-volatile function", func(t *testing.T) {
		registry := newScalarFuncRegistry()
		...
		err = registry.register("non_volatile", fn)
		require.NoError(t, err)

		udfInfo, _, found := registry.LookupScalarUDF("non_volatile", []Type{TYPE_INTEGER})
		require.True(t, found)
	...
	t.Run("IsVolatile returns false for invalid udfInfo", func(t *testing.T) {
		registry := newScalarFuncRegistry()

		isVolatile := registry.IsVolatile("not a udf")
		require.False(t, isVolatile)
	})

	t.Run("BindScalarUDF skips ScalarBinder for volatile function", func(t *testing.T) {
		registry := newScalarFuncRegistry()
		...
		err = registry.register("volatile_with_binder", fn)
		require.NoError(t, err)

		udfInfo, _, found := registry.LookupScalarUDF("volatile_with_binder", []Type{TYPE_INTEGER})
		require.True(t, found)
		...
		bindCtx, err := registry.BindScalarUDF(udfInfo, args)
		require.NoError(t, err)
		require.Nil(t, bindCtx)        // Should return nil for volatile functions
		require.False(t, binderCalled) // ScalarBinder should not be called
	})
}
...
func BenchmarkScalarFuncRegistration(b *testing.B) {
	intType, err := NewTypeInfo(TYPE_INTEGER)
	if err != nil {
		b.Fatal(err)
	}
...
func BenchmarkScalarFuncLookup(b *testing.B) {
	...
	for i := range 10 {
		...
		_ = registry.register(fmt.Sprintf("func_%d", i), fn)
	}
...
func BenchmarkScalarFuncExecution(b *testing.B) {
	...
	udf := registry.lookup("add", []Type{TYPE_INTEGER, TYPE_INTEGER})
...
func BenchmarkScalarFuncTypeMatching(b *testing.B) {
	registry := newScalarFuncRegistry()
	...
	_ = registry.register("overloaded", createSimpleScalarFunc([]TypeInfo{intType}, intType,
		func(v []driver.Value) (any, error) { return v[0], nil }))
	_ = registry.register("overloaded", createSimpleScalarFunc([]TypeInfo{strType}, strType,
		func(v []driver.Value) (any, error) { return v[0], nil }))
	_ = registry.register("overloaded", createSimpleScalarFunc([]TypeInfo{floatType}, floatType,
		func(v []driver.Value) (any, error) { return v[0], nil }))
	_ = registry.register("overloaded", createSimpleScalarFunc([]TypeInfo{intType, intType}, intType,
		func(v []driver.Value) (any, error) { return v[0], nil }))
...
func BenchmarkScalarFuncBinderCall(b *testing.B) {
	...
	fn := &testScalarFuncWithBinder{
		...
		executor: ScalarFuncExecutor{
			...
			ScalarBinder: func(ctx context.Context, args []ScalarUDFArg) (context.Context, error) {
				return context.WithValue(ctx, testContextKey("key"), args[0].Value), nil
			},
		},
	}

	_ = registry.register("with_binder", fn)
	udfInfo, _, _ := registry.LookupScalarUDF("with_binder", []Type{TYPE_INTEGER})
	args := []ScalarUDFArg{{Foldable: true, Value: int32(42)}}
