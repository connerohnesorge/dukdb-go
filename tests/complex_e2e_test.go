...
func ceToInt64(v any) int64 {
	...
	case nil:
		return 0
	default:
		// Best-effort conversion for unexpected types; panics on failure for debugging
		n := int64(0)
		if _, err := fmt.Sscan(fmt.Sprintf("%v", x), &n); err != nil {
...
func ceToFloat64(v any) float64 {
	...
	default:
		// Best-effort conversion for unexpected types; panics on failure for debugging
		f := 0.0
		if _, err := fmt.Sscan(fmt.Sprintf("%v", x), &f); err != nil {
