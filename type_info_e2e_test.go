...
func TestTypeInfoEndToEnd(t *testing.T) {
	t.Run(
		"complete workflow for all type categories",
		func(t *testing.T) {
			...
			for _, typ := range primitiveTypes {
				...
				assert.Nil(
					t,
					info.Details(),
					"Primitive types should have nil Details()",
				)
				assert.NotEmpty(
					t,
					info.SQLType(),
					"SQLType() should return non-empty string",
				)
			}
			...
			decimalInfo, err := NewDecimalInfo(
				18,
				6,
			)
	...
	t.Run(
		...
		func(t *testing.T) {
			...
			innerStructInfo, err := NewStructInfo(
				keyEntry,
				valueEntry,
			)
			require.NoError(t, err)
			...
			tagsEntry, _ := NewStructEntry(
				tagsInfo,
				"tags",
			)
			metadataEntry, _ := NewStructEntry(
				metadataInfo,
				"metadata",
			)
			outerStructInfo, err := NewStructInfo(
				idEntry,
				tagsEntry,
				metadataEntry,
			)
			...
			assert.Equal(
				t,
				TYPE_STRUCT,
				metaDetails.Value.InternalType(),
			)
		},
	)
