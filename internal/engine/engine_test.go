...
func TestStatementType(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			dukdb.STATEMENT_TYPE_SELECT,
		},
		{
			"INSERT",
			"INSERT INTO test_types VALUES (1, 'test')",
			dukdb.STATEMENT_TYPE_INSERT,
		},
		{
			"UPDATE",
			"UPDATE test_types SET name = 'updated' WHERE id = 1",
			dukdb.STATEMENT_TYPE_UPDATE,
		},
		{
			"DELETE",
			"DELETE FROM test_types WHERE id = 1",
			dukdb.STATEMENT_TYPE_DELETE,
		},
		{
			"CREATE",
			"CREATE TABLE new_table (id INTEGER)",
			dukdb.STATEMENT_TYPE_CREATE,
		},
		{
			"DROP",
			"DROP TABLE IF EXISTS new_table",
			dukdb.STATEMENT_TYPE_DROP,
		},
	}
	...
	for _, tc := range testCases {
		...
		require.NoError(t, stmt.Close())
			...
			if !ok {
				t.Fatal(
					"Statement does not implement BackendStmtIntrospector",
				)
			}
...
func TestStatementTypeClosedStatement(
	t *testing.T,
) {
	engine := NewEngine()
	defer func() {
	...
	if stmtType != dukdb.STATEMENT_TYPE_INVALID {
		t.Errorf(
			"StatementType() on closed = %v, want INVALID",
			stmtType,
		)
	}
}
...
func TestColumnMetadata(t *testing.T) {
	...
	t.Run(
		...
		require.NoError(t, stmt.Close())
			...
			if count != 0 {
				t.Errorf(
					"ColumnCount() for INSERT = %d, want 0",
					count,
				)
			}
		},
...
func TestParamNameAndType(t *testing.T) {
	...
	if outOfBounds != dukdb.TYPE_INVALID {
		t.Errorf(
			"ParamType(10) = %v, want TYPE_INVALID",
			outOfBounds,
		)
	}
}

func TestStatementIntrospectionWithAliases(
	t *testing.T,
) {
	engine := NewEngine()
	defer func() {
...
func TestBoundExecution(t *testing.T) {
	...
	t.Run(
		...
		func(t *testing.T) {
			stmt, err := conn.Prepare(
				context.Background(),
				"SELECT * FROM test_bound WHERE id = $1",
			)
			if err != nil {
				t.Fatalf(
					"Prepare failed: %v",
	...
	t.Run(
		...
		func(t *testing.T) {
			stmt, err := conn.Prepare(
				context.Background(),
				"INSERT INTO test_bound VALUES ($1, $2)",
			)
			if err != nil {
				t.Fatalf(
					"Prepare failed: %v",
...
func TestColumnTypeInfo(t *testing.T) {
	...
	t.Run(
		"valid index returns TypeInfo",
		func(t *testing.T) {
			// INTEGER column
			typeInfo0 := intro.ColumnTypeInfo(0)
			if typeInfo0 == nil {
				t.Fatal(
					"ColumnTypeInfo(0) returned nil",
				)
			}
			if typeInfo0.InternalType() != dukdb.TYPE_INTEGER {
				t.Errorf(
					"ColumnTypeInfo(0).InternalType() = %v, want INTEGER",
			...
			if typeInfo1 == nil {
				t.Fatal(
					"ColumnTypeInfo(1) returned nil",
				)
			}
			if typeInfo1.InternalType() != dukdb.TYPE_VARCHAR {
				t.Errorf(
					"ColumnTypeInfo(1).InternalType() = %v, want VARCHAR",
			...
			if typeInfo2 == nil {
				t.Fatal(
					"ColumnTypeInfo(2) returned nil",
				)
			}
			if typeInfo2.InternalType() != dukdb.TYPE_DOUBLE {
				t.Errorf(
					"ColumnTypeInfo(2).InternalType() = %v, want DOUBLE",
			...
			if typeInfo3 == nil {
				t.Fatal(
					"ColumnTypeInfo(3) returned nil",
				)
			}
			if typeInfo3.InternalType() != dukdb.TYPE_BOOLEAN {
				t.Errorf(
					"ColumnTypeInfo(3).InternalType() = %v, want BOOLEAN",
					typeInfo3.InternalType(),
				)
			}
		},
	)
	...
	t.Run(
		"primitive types have nil details",
		func(t *testing.T) {
			typeInfo := intro.ColumnTypeInfo(0)
			if typeInfo.Details() != nil {
				t.Errorf(
					"ColumnTypeInfo(0).Details() = %v, want nil for primitive type",
					typeInfo.Details(),
				)
			}
		},
...
func TestParameterTypeInference(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			"WHERE value > ?",
			"SELECT * FROM test_types WHERE value > ?",
			1,
			dukdb.TYPE_DOUBLE,
		},
		...
		{
			"INSERT id",
			"INSERT INTO test_types (id) VALUES (?)",
			1,
			dukdb.TYPE_INTEGER,
		},
		{
		...
		{
			"INSERT value",
			"INSERT INTO test_types (value) VALUES (?)",
			1,
			dukdb.TYPE_DOUBLE,
		},
		...
		{
			"UPDATE SET value",
			"UPDATE test_types SET value = ? WHERE id = 1",
			1,
			dukdb.TYPE_DOUBLE,
		},
		{
			"UPDATE SET name",
			"UPDATE test_types SET name = ? WHERE id = 1",
			1,
			dukdb.TYPE_VARCHAR,
		},
		{
			"UPDATE WHERE id",
			"UPDATE test_types SET name = 'x' WHERE id = ?",
			1,
			dukdb.TYPE_INTEGER,
		},
		...
		{
			"LIMIT",
			"SELECT * FROM test_types LIMIT ?",
			1,
			dukdb.TYPE_BIGINT,
		},
		{
			"OFFSET",
			"SELECT * FROM test_types LIMIT 10 OFFSET ?",
			1,
			dukdb.TYPE_BIGINT,
		},
	}
	...
	for _, tc := range testCases {
		...
		require.NoError(t, stmt.Close())
			...
			if actualType != tc.expectedType {
				t.Errorf(
					"ParamType(%d) = %v, want %v for SQL: %s",
					tc.paramIndex,
					actualType,
					tc.expectedType,
					tc.sql,
...
func TestParameterTypeInferenceBETWEEN(
	t *testing.T,
) {
	engine := NewEngine()
	defer func() {
	...
	if type1 != dukdb.TYPE_INTEGER {
		t.Errorf(
			"ParamType(1) = %v, want INTEGER for BETWEEN low bound",
			type1,
		)
	}
	...
	if type2 != dukdb.TYPE_INTEGER {
		t.Errorf(
			"ParamType(2) = %v, want INTEGER for BETWEEN high bound",
			type2,
		)
	}
}

func TestParameterTypeInferenceINList(
	t *testing.T,
) {
	engine := NewEngine()
	defer func() {
	...
	for i := 1; i <= 3; i++ {
		...
		if typ != dukdb.TYPE_VARCHAR {
			t.Errorf(
				"ParamType(%d) = %v, want VARCHAR for IN list value",
				i,
				typ,
			)
		}
