...
func TestVectorPool_GetFromEmpty(t *testing.T) {
	...
	t.Run(
		"returned vector has all NULL validity",
		func(t *testing.T) {
			pool := NewVectorPool()
			v := pool.Get(testTypeInteger, 2048)
	...
	t.Run(
		"multiple gets create new vectors",
		func(t *testing.T) {
			pool := NewVectorPool()
...
func TestVectorPool_ReturnAndReuse(t *testing.T) {
	...
	t.Run(
		"put vector with nil mask does nothing",
		func(_ *testing.T) {
			pool := NewVectorPool()
			v := &PooledVector{
				Mask:     nil,
...
func TestVectorPool_TypeSpecific(t *testing.T) {
	t.Run(
		"different types use different pools",
		func(t *testing.T) {
			pool := NewVectorPool()
			...
			assert.Equal(
				t,
				uint64(0),
				vInt2.Mask.CountValid(),
			)
			assert.Equal(
				t,
				uint64(0),
				vStr2.Mask.CountValid(),
			)
	...
	t.Run(
		"different capacities create new vectors",
		func(t *testing.T) {
			pool := NewVectorPool()
...
func TestVectorPool_Concurrent(t *testing.T) {
	t.Run(
		"concurrent get and put operations",
		func(t *testing.T) {
			pool := NewVectorPool()
	...
	t.Run(
		"concurrent operations on different types",
		func(t *testing.T) {
			pool := NewVectorPool()
...
func BenchmarkVectorPool_GetPutParallel(
	b *testing.B,
) {
	pool := NewVectorPool()
	b.ResetTimer()
