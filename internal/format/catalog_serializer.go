...
func ValidateHeader(r io.Reader) error {
	...
	if magic != DuckDBMagicNumber {
		return fmt.Errorf("%w: got 0x%08X, expected 0x%08X", ErrInvalidMagicNumber, magic, DuckDBMagicNumber)
	}
	...
	if version != DuckDBFormatVersion {
		return fmt.Errorf("%w: got %d, expected %d", ErrUnsupportedVersion, version, DuckDBFormatVersion)
	}
...
func SerializeCatalog(w io.Writer, cat *catalog.Catalog) error {
	if cat == nil {
		return fmt.Errorf("cannot serialize nil catalog")
	}
	...
	if err := binary.Write(w, ByteOrder, uint64(len(schemas))); err != nil {
		return fmt.Errorf("failed to write schema count: %w", err)
	}

	// Serialize each schema
	for i, schema := range schemas {
		if err := SerializeSchema(w, schema); err != nil {
			return fmt.Errorf("failed to serialize schema %d: %w", i, err)
		}
	}
...
func SerializeSchema(w io.Writer, schema *catalog.Schema) error {
	if schema == nil {
		return fmt.Errorf("cannot serialize nil schema")
	}
	...
	if err := bw.WriteProperty(100, uint32(CatalogEntryType_SCHEMA)); err != nil {
		return fmt.Errorf("failed to write schema entry type: %w", err)
	}

	// Property 101: Schema name
	if err := bw.WriteProperty(101, schema.Name()); err != nil {
		return fmt.Errorf("failed to write schema name: %w", err)
	}
	...
	if err := bw.WriteProperty(200, uint64(len(tables))); err != nil {
		return fmt.Errorf("failed to write table count: %w", err)
	}

	// Flush schema properties
	if err := bw.Flush(); err != nil {
		return fmt.Errorf("failed to flush schema properties: %w", err)
	}

	// Serialize each table
	for i, table := range tables {
		if err := SerializeTableEntry(w, table); err != nil {
			return fmt.Errorf("failed to serialize table %d (%s): %w", i, table.Name, err)
		}
	}
...
func SerializeTableEntry(w io.Writer, table *catalog.TableDef) error {
	if table == nil {
		return fmt.Errorf("cannot serialize nil table")
	}
	...
	if err := bw.WriteProperty(100, uint32(CatalogEntryType_TABLE)); err != nil {
		return fmt.Errorf("failed to write table entry type: %w", err)
	}
	...
	if err := bw.Flush(); err != nil {
		return fmt.Errorf("failed to flush table properties: %w", err)
	}

	// Serialize each column
	for i, col := range table.Columns {
		if err := SerializeColumn(w, col); err != nil {
			return fmt.Errorf("failed to serialize column %d (%s): %w", i, col.Name, err)
		}
	}
...
func SerializeColumn(w io.Writer, col *catalog.ColumnDef) error {
	if col == nil {
		return fmt.Errorf("cannot serialize nil column")
	}
	...
	if err := SerializeTypeInfo(typeInfoWriter, typeInfo); err != nil {
		return fmt.Errorf("failed to serialize column TypeInfo: %w", err)
	}
	if err := typeInfoWriter.Flush(); err != nil {
		return fmt.Errorf("failed to flush column TypeInfo: %w", err)
	}

	// Write the serialized TypeInfo as property 101
	if err := bw.WriteProperty(101, typeInfoBuf.Bytes()); err != nil {
		return fmt.Errorf("failed to write column TypeInfo property: %w", err)
	}
	...
	if !col.Nullable {
		if err := bw.WriteProperty(102, uint8(0)); err != nil {
			return fmt.Errorf("failed to write nullable flag: %w", err)
		}
	}
	...
	if err := bw.Flush(); err != nil {
		return fmt.Errorf("failed to flush column properties: %w", err)
	}
