...
func TestCatalogSaveLoadRoundTrip(t *testing.T) {
	...
	require.NoError(t, err, "Failed to create primitive table")

	// Create a table with complex types
	complexTable := createComplexTable(t)
	err = originalCat.CreateTable(complexTable)
	require.NoError(t, err, "Failed to create complex table")

	// Create a table with nested complex types
	nestedTable := createNestedTable(t)
	err = originalCat.CreateTable(nestedTable)
	require.NoError(t, err, "Failed to create nested table")

	// Create a custom schema with tables
	_, err = originalCat.CreateSchema("custom_schema")
	require.NoError(t, err, "Failed to create custom schema")

	customTable := createCustomSchemaTable(t)
	err = originalCat.CreateTableInSchema("custom_schema", customTable)
	require.NoError(t, err, "Failed to create table in custom schema")

	// Save the catalog to disk using the format package
	err = format.SaveCatalogToDuckDBFormat(originalCat, dbPath)
	require.NoError(t, err, "Failed to save catalog")
...
func createComplexTable(t *testing.T) *catalog.TableDef {
	// Create DECIMAL type
	...
	// Create STRUCT type
	streetEntry, err := dukdb.NewStructEntry(mustTypeInfo(t, dukdb.TYPE_VARCHAR), "street")
	require.NoError(t, err)
	cityEntry, err := dukdb.NewStructEntry(mustTypeInfo(t, dukdb.TYPE_VARCHAR), "city")
	require.NoError(t, err)
	zipcodeEntry, err := dukdb.NewStructEntry(mustTypeInfo(t, dukdb.TYPE_INTEGER), "zipcode")
	require.NoError(t, err)
	structInfo, err := dukdb.NewStructInfo(streetEntry, cityEntry, zipcodeEntry)
	require.NoError(t, err)

	// Create ARRAY type (ARRAY of VARCHARs, size 10)
	varcharInfo, err := dukdb.NewTypeInfo(dukdb.TYPE_VARCHAR)
	require.NoError(t, err)
	arrayInfo, err := dukdb.NewArrayInfo(varcharInfo, 10)
	require.NoError(t, err)
	...
	return catalog.NewTableDef("products", columns)
}

// createNestedTable creates a table with deeply nested complex types.
func createNestedTable(t *testing.T) *catalog.TableDef {
	// Create nested LIST: LIST(LIST(INTEGER))
	intInfo, err := dukdb.NewTypeInfo(dukdb.TYPE_INTEGER)
	require.NoError(t, err)
	innerListInfo, err := dukdb.NewListInfo(intInfo)
	require.NoError(t, err)
	outerListInfo, err := dukdb.NewListInfo(innerListInfo)
	require.NoError(t, err)
	...
	countEntry, err := dukdb.NewStructEntry(mustTypeInfo(t, dukdb.TYPE_INTEGER), "count")
	require.NoError(t, err)
	totalEntry, err := dukdb.NewStructEntry(mustTypeInfo(t, dukdb.TYPE_DOUBLE), "total")
	require.NoError(t, err)
	valueStructInfo, err := dukdb.NewStructInfo(countEntry, totalEntry)
	require.NoError(t, err)

	mapInfo, err := dukdb.NewMapInfo(varcharInfo, valueStructInfo)
	require.NoError(t, err)
	...
	nameEntry, err := dukdb.NewStructEntry(mustTypeInfo(t, dukdb.TYPE_VARCHAR), "name")
	require.NoError(t, err)
	amountEntry, err := dukdb.NewStructEntry(decimalInfo, "amount")
	require.NoError(t, err)
	tagsEntry, err := dukdb.NewStructEntry(innerListInfo, "tags")
	require.NoError(t, err)
	nestedStructInfo, err := dukdb.NewStructInfo(nameEntry, amountEntry, tagsEntry)
	require.NoError(t, err)
	...
	return catalog.NewTableDef("analytics", columns)
}

// createCustomSchemaTable creates a table for a custom schema.
func createCustomSchemaTable(_ *testing.T) *catalog.TableDef {
	columns := []*catalog.ColumnDef{
		catalog.NewColumnDef("id", dukdb.TYPE_INTEGER).WithNullable(false),
...
func mustTypeInfo(t *testing.T, typ dukdb.Type) dukdb.TypeInfo {
...
func validateCatalogMatch(t *testing.T, original, loaded *catalog.Catalog) {
	...
	require.Equal(t, len(originalSchemas), len(loadedSchemas), "Schema count mismatch")

	// Create maps for easier lookup
	originalSchemaMap := make(map[string]*catalog.Schema)
	for _, s := range originalSchemas {
		originalSchemaMap[s.Name()] = s
	}

	loadedSchemaMap := make(map[string]*catalog.Schema)
	...
	for schemaName, origSchema := range originalSchemaMap {
		...
		require.True(t, exists, "Schema %s not found in loaded catalog", schemaName)

		// Compare tables in each schema
		origTables := origSchema.ListTables()
		loadedTables := loadedSchema.ListTables()

		require.Equal(t, len(origTables), len(loadedTables),
			"Table count mismatch in schema %s", schemaName)

		// Create maps for easier lookup
		origTableMap := make(map[string]*catalog.TableDef)
		for _, t := range origTables {
			origTableMap[t.Name] = t
		}

		loadedTableMap := make(map[string]*catalog.TableDef)
		...
		for tableName, origTable := range origTableMap {
			...
			require.True(t, exists, "Table %s not found in loaded schema %s",
				tableName, schemaName)

			validateTableMatch(t, origTable, loadedTable)
		}
	}
}

// validateTableMatch compares two table definitions to ensure they match.
func validateTableMatch(t *testing.T, original, loaded *catalog.TableDef) {
	assert.Equal(t, original.Name, loaded.Name, "Table name mismatch")
	assert.Equal(t, original.Schema, loaded.Schema, "Schema name mismatch")
	require.Equal(t, len(original.Columns), len(loaded.Columns),
		"Column count mismatch for table %s", original.Name)

	// Compare each column
	for i, origCol := range original.Columns {
		loadedCol := loaded.Columns[i]
		validateColumnMatch(t, origCol, loadedCol, original.Name)
	}
}

// validateColumnMatch compares two column definitions to ensure they match.
func validateColumnMatch(t *testing.T, original, loaded *catalog.ColumnDef, tableName string) {
	assert.Equal(t, original.Name, loaded.Name,
		"Column name mismatch in table %s", tableName)
	assert.Equal(t, original.Type, loaded.Type,
		"Column type mismatch for %s.%s", tableName, original.Name)
	assert.Equal(t, original.Nullable, loaded.Nullable,
		"Nullable mismatch for %s.%s", tableName, original.Name)
	...
	require.NotNil(t, loaded.TypeInfo,
		"TypeInfo missing for %s.%s", tableName, original.Name)
	loc := typeLocation{tableName, original.Name}
	validateTypeInfoMatch(t, original.TypeInfo, loaded.TypeInfo, loc)
}
...
func validateTypeInfoMatch(t *testing.T, original, loaded dukdb.TypeInfo, loc typeLocation) {
	assert.Equal(t, original.InternalType(), loaded.InternalType(),
		"TypeInfo InternalType mismatch for %s", loc)
	...
	if origDetails == nil {
		assert.Nil(t, loadedDetails,
			"Loaded TypeInfo has details but original doesn't for %s", loc)

		return
	}

	require.NotNil(t, loadedDetails,
		"Original TypeInfo has details but loaded doesn't for %s", loc)
...
func TestCatalogSaveLoadEmptyCatalog(t *testing.T) {
	tmpDir := t.TempDir()
	dbPath := filepath.Join(tmpDir, "empty.duckdb")
	...
	assert.Equal(t, 0, len(schemas[0].ListTables()))
}

// TestCatalogLoadNonexistentFile tests loading from a file that doesn't exist.
func TestCatalogLoadNonexistentFile(t *testing.T) {
	_, err := format.LoadCatalogFromDuckDBFormat("/nonexistent/path/to/file.duckdb")
	require.Error(t, err)
	assert.Contains(t, err.Error(), "failed to open catalog file")
}

// TestCatalogSaveInvalidPath tests saving to an invalid path.
func TestCatalogSaveInvalidPath(t *testing.T) {
	cat := catalog.NewCatalog()
	err := format.SaveCatalogToDuckDBFormat(cat, "/invalid/path/that/does/not/exist/file.duckdb")
	require.Error(t, err)
	assert.Contains(t, err.Error(), "failed to create catalog file")
}
