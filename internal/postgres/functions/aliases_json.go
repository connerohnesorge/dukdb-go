...
func (r *FunctionAliasRegistry) registerJSONFuncs() {
	aliases := []*FunctionAlias{
		{
			PostgreSQLName: "json_array_length",
			DuckDBName:     "json_array_length",
			Category:       DirectAlias,
			MinArgs:        1,
			MaxArgs:        1,
			Description:    "Returns JSON array length",
		},
		{
			PostgreSQLName: "jsonb_array_length",
			DuckDBName:     "json_array_length",
			Category:       DirectAlias,
			MinArgs:        1,
			MaxArgs:        1,
			Description:    "Returns JSONB array length",
		},
		...
		{
			PostgreSQLName: "json_extract_path_text",
			DuckDBName:     "json_extract_string",
			Category:       DirectAlias,
			MinArgs:        2,
			MaxArgs:        -1,
			Description:    "Extracts JSON value as text",
		},
		{
			PostgreSQLName: "jsonb_extract_path_text",
			DuckDBName:     "json_extract_string",
			Category:       DirectAlias,
			MinArgs:        2,
			MaxArgs:        -1,
			Description:    "Extracts JSONB value as text",
		},
