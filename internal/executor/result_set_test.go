...
func TestResultSet_Next_SingleChunk(t *testing.T) {
...
func TestResultSet_Next_MultipleChunks(t *testing.T) {
	types := []dukdb.Type{dukdb.TYPE_INTEGER, dukdb.TYPE_VARCHAR}
	...
	for i, exp := range expected {
		...
		assert.Equal(t, exp.id, dest[0], "row %d id", i)
		assert.Equal(t, exp.name, dest[1], "row %d name", i)
	}
...
func TestResultSet_Next_EmptyChunks(t *testing.T) {
	var chunks []*storage.DataChunk
	columnNames := []string{"id", "name"}
	var typeInfos []dukdb.TypeInfo
...
func TestResultSet_Next_VariousTypes(t *testing.T) {
	types := []dukdb.Type{
		dukdb.TYPE_BOOLEAN,
		dukdb.TYPE_TINYINT,
		dukdb.TYPE_SMALLINT,
	...
	chunks := []*storage.DataChunk{chunk}
	columnNames := []string{"bool", "i8", "i16", "i32", "i64", "f32", "f64", "str"}
	var typeInfos []dukdb.TypeInfo
...
func TestResultSet_ImplementsDriverRows(_ *testing.T) {
...
func TestResultSet_Next_DestinationTooSmall(t *testing.T) {
	types := []dukdb.Type{dukdb.TYPE_INTEGER, dukdb.TYPE_VARCHAR}
	chunk := storage.NewDataChunk(types)
	chunk.AppendRow([]any{int32(1), "Alice"})
...
func TestResultSet_Next_EmptyChunkInMiddle(t *testing.T) {
	types := []dukdb.Type{dukdb.TYPE_INTEGER}
	...
	chunks := []*storage.DataChunk{chunk1, chunk2, chunk3}
	columnNames := []string{"id"}
	var typeInfos []dukdb.TypeInfo
...
func TestResultSet_ColumnTypeDatabaseTypeName(t *testing.T) {
	...
	decimalType, err := dukdb.NewDecimalInfo(10, 2)
	require.NoError(t, err)

	typeInfos := []dukdb.TypeInfo{intType, varcharType, decimalType}
	...
	assert.Equal(t, "", rs.ColumnTypeDatabaseTypeName(100))
}

// TestResultSet_ColumnTypeDatabaseTypeName_ComplexTypes tests complex type names.
func TestResultSet_ColumnTypeDatabaseTypeName_ComplexTypes(t *testing.T) {
	// Create a LIST type
	intType, err := dukdb.NewTypeInfo(dukdb.TYPE_INTEGER)
	require.NoError(t, err)
	...
	arrayType, err := dukdb.NewArrayInfo(intType, 5)
	require.NoError(t, err)
	...
	typeInfos := []dukdb.TypeInfo{listType, arrayType, mapType}
	columnNames := []string{"int_list", "int_array", "str_int_map"}
	...
	assert.Equal(t, "INTEGER[]", rs.ColumnTypeDatabaseTypeName(0))
	assert.Equal(t, "INTEGER[5]", rs.ColumnTypeDatabaseTypeName(1))
	assert.Equal(t, "MAP(VARCHAR, INTEGER)", rs.ColumnTypeDatabaseTypeName(2))
}
...
func TestResultSet_ColumnTypeInfo_ComplexTypes(t *testing.T) {
	// Create a LIST type
	intType, err := dukdb.NewTypeInfo(dukdb.TYPE_INTEGER)
	require.NoError(t, err)
	...
	field1, err := dukdb.NewStructEntry(intType, "x")
	require.NoError(t, err)

	varcharType, err := dukdb.NewTypeInfo(dukdb.TYPE_VARCHAR)
	require.NoError(t, err)

	field2, err := dukdb.NewStructEntry(varcharType, "y")
	require.NoError(t, err)

	structType, err := dukdb.NewStructInfo(field1, field2)
	require.NoError(t, err)

	typeInfos := []dukdb.TypeInfo{listType, structType}
	...
	require.True(t, ok)
	assert.Equal(t, dukdb.TYPE_INTEGER, listDetailsTyped.Child.InternalType())
	...
	assert.Equal(t, "x", structDetailsTyped.Entries[0].Name())
	assert.Equal(t, "y", structDetailsTyped.Entries[1].Name())
}

// TestResultSet_ImplementsRowsColumnTypeDatabaseTypeName verifies compile-time interface compliance.
func TestResultSet_ImplementsRowsColumnTypeDatabaseTypeName(_ *testing.T) {
