...
// JSONMultiFileOptions contains options for multi-file JSON reading.
type JSONMultiFileOptions struct {
...
// DefaultJSONMultiFileOptions returns the default options for multi-file JSON reading.
func DefaultJSONMultiFileOptions() *JSONMultiFileOptions {
	return &JSONMultiFileOptions{
		Filename:          false,
		FileRowNumber:     false,
		FileIndex:         false,
		FilesToSniff:      1,
...
func (e *Executor) executeReadJSON(
	...
	multiOpts := DefaultJSONMultiFileOptions()
	...
	if plan.TableFunction != nil && len(plan.TableFunction.Paths) > 0 {
		// Array syntax: read_json(['file1.json', 'file2.json'])
		// Expand any glob patterns within the array
		for _, p := range plan.TableFunction.Paths {
			if fileio.IsGlobPattern(p) {
				expanded, err := e.expandJSONGlobPattern(ctx.Context, p, multiOpts)
				if err != nil {
					return nil, err
				}
		...
		paths, err = e.resolveJSONFilePaths(ctx.Context, plan.Path, multiOpts)
		if err != nil {
			return nil, err
		}
	}
	...
	return e.executeReadJSONMultiFile(ctx, paths, opts, multiOpts, plan)
}
...
// executeReadJSONSingleFile executes read_json for a single file (optimized path).
func (e *Executor) executeReadJSONSingleFile(
	ctx *ExecutionContext,
	path string,
...
// executeReadJSONMultiFile executes read_json for multiple files with schema merging.
func (e *Executor) executeReadJSONMultiFile(
	ctx *ExecutionContext,
	paths []string,
	opts *json.ReaderOptions,
	...
	for i, path := range paths {
		if i < filesToSniff {
			...
			schema, err := e.sniffJSONSchema(ctx.Context, path, opts)
			if err != nil {
				return nil, &dukdb.Error{
					Type: dukdb.ErrorTypeIO,
	...
	if isJSONHivePartitioningEnabled(multiOpts) {
	...
	for fileIdx, path := range paths {
		// Read this file
		chunks, err := e.readJSONFileChunks(ctx.Context, path, opts)
		if err != nil {
			return nil, &dukdb.Error{
				Type: dukdb.ErrorTypeIO,
...
func (e *Executor) sniffJSONSchema(ctx context.Context, path string, opts *json.ReaderOptions) (fileio.FileSchema, error) {
	...
	if filesystem.IsCloudURL(path) {
		...
		if err != nil {
			return fileio.FileSchema{}, fmt.Errorf("failed to open JSON file: %w", err)
		}
		closer = file
		reader, err = json.NewReader(file, opts)
		if err != nil {
			_ = file.Close()
			return fileio.FileSchema{}, fmt.Errorf("failed to create JSON reader: %w", err)
		}
	} else {
		file, err := os.Open(path)
		if err != nil {
...
func (e *Executor) readJSONFileChunks(ctx context.Context, path string, opts *json.ReaderOptions) ([]*storage.DataChunk, error) {
	...
	if filesystem.IsCloudURL(path) {
		...
		if err != nil {
			_ = file.Close()
			return nil, fmt.Errorf("failed to create JSON reader: %w", err)
		}
	} else {
		file, err := os.Open(path)
		if err != nil {
...
// isJSONHivePartitioningEnabled returns true if hive partitioning is enabled for JSON.
func isJSONHivePartitioningEnabled(opts *JSONMultiFileOptions) bool {
...
func (e *Executor) executeReadJSONAuto(
	...
	if plan.TableFunction != nil && len(plan.TableFunction.Paths) > 0 {
		// Array syntax: read_json_auto(['file1.json', 'file2.json'])
		for _, p := range plan.TableFunction.Paths {
			if fileio.IsGlobPattern(p) {
				expanded, err := e.expandJSONGlobPattern(ctx.Context, p, multiOpts)
				if err != nil {
...
func (e *Executor) executeReadNDJSON(
	...
	if plan.TableFunction != nil && len(plan.TableFunction.Paths) > 0 {
		// Array syntax: read_ndjson(['file1.ndjson', 'file2.ndjson'])
		for _, p := range plan.TableFunction.Paths {
			if fileio.IsGlobPattern(p) {
				expanded, err := e.expandJSONGlobPattern(ctx.Context, p, multiOpts)
				if err != nil {
