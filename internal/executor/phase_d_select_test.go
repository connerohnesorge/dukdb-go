...
func TestPhaseD_LiteralSelect(t *testing.T) {
	...
	t.Run("SELECT 1", func(t *testing.T) {
		result, err := executeQuery(t, exec, cat, "SELECT 1")
		...
		assert.Greater(t, len(result.Rows[0]), 0, "should have at least one column")
	})

	t.Run("SELECT 1, 2, 3", func(t *testing.T) {
		result, err := executeQuery(t, exec, cat, "SELECT 1, 2, 3")
		require.NoError(t, err)

		require.Equal(t, 1, len(result.Rows))
		// Should have 3 columns
		assert.Equal(t, 3, len(result.Rows[0]), "should have 3 columns")
	})
}

// TestPhaseD_ArithmeticExpressions tests SELECT with arithmetic (Task 4.2)
func TestPhaseD_ArithmeticExpressions(t *testing.T) {
	exec, cat, _ := setupTestExecutor()

	t.Run("SELECT 1 + 2", func(t *testing.T) {
		result, err := executeQuery(t, exec, cat, "SELECT 1 + 2")
		require.NoError(t, err)

		require.Equal(t, 1, len(result.Rows))
		// Result should be 3 (column name may vary)
		assert.Greater(t, len(result.Rows[0]), 0, "should have result column")
	})
	...
	t.Run("SELECT (10 - 5) * 2", func(t *testing.T) {
		result, err := executeQuery(t, exec, cat, "SELECT (10 - 5) * 2")
		require.NoError(t, err)
...
func TestPhaseD_TableScan(t *testing.T) {
	...
	tableDef := catalog.NewTableDef("users", []*catalog.ColumnDef{
		...
		{Name: "age", Type: dukdb.TYPE_INTEGER, Nullable: false},
	})
	err := cat.CreateTableInSchema("main", tableDef)
	require.NoError(t, err)

	table, err := stor.CreateTable("users", []dukdb.Type{dukdb.TYPE_INTEGER, dukdb.TYPE_VARCHAR, dukdb.TYPE_INTEGER})
	require.NoError(t, err)
	...
	for _, row := range testData {
		err = table.AppendRow([]any{row.id, row.name, row.age})
		require.NoError(t, err)
	}
	...
	t.Run("SELECT id, name FROM users", func(t *testing.T) {
		...
		projectPlan := &planner.PhysicalProject{
			...
			Expressions: []binder.BoundExpr{
				&binder.BoundColumnRef{Table: "users", Column: "id", ColType: dukdb.TYPE_INTEGER},
				&binder.BoundColumnRef{Table: "users", Column: "name", ColType: dukdb.TYPE_VARCHAR},
			},
			Aliases: []string{"id", "name"},
		}

		ctx := context.Background()
		result, err := exec.Execute(ctx, projectPlan, nil)
		require.NoError(t, err)

		require.Equal(t, 5, len(result.Rows))
		// Verify only id and name columns present
		assert.Contains(t, result.Rows[0], "id")
		assert.Contains(t, result.Rows[0], "name")
		assert.NotContains(t, result.Rows[0], "age")
	})
}
...
func TestPhaseD_FilteredQueries(t *testing.T) {
	...
	tableDef := catalog.NewTableDef("products", []*catalog.ColumnDef{
		{Name: "id", Type: dukdb.TYPE_INTEGER, Nullable: false},
		{Name: "price", Type: dukdb.TYPE_INTEGER, Nullable: false},
		{Name: "category", Type: dukdb.TYPE_VARCHAR, Nullable: false},
	})
	err := cat.CreateTableInSchema("main", tableDef)
	require.NoError(t, err)

	table, err := stor.CreateTable("products", []dukdb.Type{dukdb.TYPE_INTEGER, dukdb.TYPE_INTEGER, dukdb.TYPE_VARCHAR})
	require.NoError(t, err)
	...
	for _, row := range testData {
		err = table.AppendRow([]any{row.id, row.price, row.category})
		require.NoError(t, err)
	}

	t.Run("SELECT * FROM products WHERE price > 100", func(t *testing.T) {
		scanPlan := &planner.PhysicalScan{
			TableName: "products",
			Alias:     "products",
			TableDef:  tableDef,
		...
		ctx := context.Background()
		result, err := exec.Execute(ctx, filterPlan, nil)
		require.NoError(t, err)
	...
	t.Run("SELECT * FROM products WHERE category = 'Electronics'", func(t *testing.T) {
		scanPlan := &planner.PhysicalScan{
			TableName: "products",
			Alias:     "products",
			TableDef:  tableDef,
		...
		for _, row := range result.Rows {
			assert.Equal(t, "Electronics", row["category"])
		}
	})
}
...
func TestPhaseD_AggregateQueries(t *testing.T) {
	...
	table, err := stor.CreateTable("sales", []dukdb.Type{dukdb.TYPE_INTEGER})
	require.NoError(t, err)
	...
	t.Run("SELECT SUM(amount) FROM sales", func(t *testing.T) {
		scanPlan := &planner.PhysicalScan{
			TableName: "sales",
			Alias:     "sales",
			TableDef:  tableDef,
	...
	t.Run("SELECT AVG(amount) FROM sales", func(t *testing.T) {
		scanPlan := &planner.PhysicalScan{
			TableName: "sales",
			Alias:     "sales",
			TableDef:  tableDef,
...
func TestPhaseD_GroupBy(t *testing.T) {
	...
	table, err := stor.CreateTable("orders", []dukdb.Type{dukdb.TYPE_VARCHAR, dukdb.TYPE_INTEGER})
	require.NoError(t, err)
	...
	t.Run("SELECT category, COUNT(*) FROM orders GROUP BY category", func(t *testing.T) {
		scanPlan := &planner.PhysicalScan{
			TableName: "orders",
			Alias:     "orders",
			TableDef:  tableDef,
...
func TestPhaseD_OrderByAndLimit(t *testing.T) {
	...
	// Create and populate table
	tableDef := catalog.NewTableDef("scores", []*catalog.ColumnDef{
		{Name: "name", Type: dukdb.TYPE_VARCHAR, Nullable: false},
		{Name: "score", Type: dukdb.TYPE_INTEGER, Nullable: false},
	})
	err := cat.CreateTableInSchema("main", tableDef)
	require.NoError(t, err)

	table, err := stor.CreateTable("scores", []dukdb.Type{dukdb.TYPE_VARCHAR, dukdb.TYPE_INTEGER})
	require.NoError(t, err)
	...
	for _, row := range testData {
		err = table.AppendRow([]any{row.name, row.score})
		require.NoError(t, err)
	}

	t.Run("SELECT * FROM scores ORDER BY score DESC", func(t *testing.T) {
		scanPlan := &planner.PhysicalScan{
			TableName: "scores",
			Alias:     "scores",
			TableDef:  tableDef,
		...
		assert.Equal(t, int32(95), result.Rows[0]["score"])
		assert.Equal(t, int32(92), result.Rows[1]["score"])
		assert.Equal(t, int32(88), result.Rows[2]["score"])
		assert.Equal(t, int32(85), result.Rows[3]["score"])
		assert.Equal(t, int32(78), result.Rows[4]["score"])
	})

	t.Run("SELECT * FROM scores ORDER BY score DESC LIMIT 3", func(t *testing.T) {
		scanPlan := &planner.PhysicalScan{
			TableName: "scores",
			Alias:     "scores",
			TableDef:  tableDef,
	...
	t.Run("SELECT * FROM scores LIMIT 10", func(t *testing.T) {
		scanPlan := &planner.PhysicalScan{
			TableName: "scores",
			Alias:     "scores",
			TableDef:  tableDef,
