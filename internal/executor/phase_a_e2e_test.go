...
func TestPhaseA_E2E_BasicSelectStar(
	t *testing.T,
) {
	...
	require.NoError(
		t,
		table.AppendRow(
			[]any{int32(1), "Alice", 10.5},
		),
	)
	require.NoError(
		t,
		table.AppendRow(
			[]any{int32(2), "Bob", 20.0},
		),
	)
	require.NoError(
		t,
		table.AppendRow(
			[]any{int32(3), "Charlie", 30.5},
		),
	)
	...
	require.NotNil(
		t,
		chunk,
		"SELECT * should return DataChunk",
	)
...
func TestPhaseA_E2E_ColumnProjection(
	t *testing.T,
) {
	// Create storage and catalog
	stor := storage.NewStorage()
	...
	require.NotNil(
		t,
		chunk,
		"SELECT a, b should return DataChunk",
	)
...
func TestPhaseA_E2E_FilteredQuery(t *testing.T) {
	...
	require.NoError(
		t,
		table.AppendRow([]any{int32(3), "Alice"}),
	)
	require.NoError(
		t,
		table.AppendRow([]any{int32(7), "Bob"}),
	)
	require.NoError(
		t,
		table.AppendRow(
	...
	require.NotNil(
		t,
		chunk,
		"Filtered query should return DataChunk",
	)
	...
	assert.Equal(
		...
		2,
		chunk.Count(),
		"Should have 2 rows (id > 5: Bob=7, David=10)",
	)
	...
	assert.Contains(
		...
		ids,
		int32(7),
		"Should contain Bob's id (7)",
	)
	assert.Contains(
		t,
		ids,
		int32(10),
		"Should contain David's id (10)",
	)
...
func TestPhaseA_E2E_NullHandling(t *testing.T) {
	...
	assert.Nil(
		t,
		chunk.GetValue(2, 2),
		"NULL score should be preserved",
	)
	...
	scanOp2, err := NewPhysicalScanOperator(
		scanPlan2,
		stor,
	)
	require.NoError(t, err)
	...
	assert.Equal(
		...
		1,
		chunk.Count(),
		"Should have 1 row with NULL name",
	)
	assert.Equal(
		t,
		int32(2),
	...
	scanOp3, err := NewPhysicalScanOperator(
		scanPlan3,
		stor,
	)
	require.NoError(t, err)
	...
	filterOp2 := NewPhysicalFilterOperator(
		scanOp3,
		childColumns,
		filterPredicate2,
		exec,
...
func TestPhaseA_E2E_ScanFilterProjectPipeline(
	t *testing.T,
) {
	// Create storage and catalog
	stor := storage.NewStorage()
	...
	require.NotNil(
		t,
		chunk,
		"Pipeline should return DataChunk",
	)
	...
	assert.Equal(
		...
		1,
		chunk.Count(),
		"Should have 1 row (Charlie only)",
	)
...
func TestPhaseA_E2E_MultipleChunks(t *testing.T) {
	...
	assert.Equal(
		...
		expectedRows,
		totalRows,
		"Should have filtered to even numbers only",
	)
...
func TestPhaseA_E2E_TypeInfoPropagation(
	t *testing.T,
) {
	// Create storage and catalog
	stor := storage.NewStorage()
