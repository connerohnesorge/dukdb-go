...
func TestPhaseB_E2E_ResultSetFromScan(t *testing.T) {
	...
	// Insert test data
	require.NoError(t, table.AppendRow([]any{int32(1), "Alice", 50000.0}))
	require.NoError(t, table.AppendRow([]any{int32(2), "Bob", 60000.0}))
	require.NoError(t, table.AppendRow([]any{int32(3), "Charlie", 75000.0}))
	...
	col3 := catalog.NewColumnDef("salary", dukdb.TYPE_DOUBLE)
	...
	columnNames := []string{"id", "name", "salary"}
	typeInfos := scanOp.GetTypes()
	rs := NewResultSet(chunks, typeInfos, columnNames)

	// Test Columns() method (Task 2.23)
	columns := rs.Columns()
	assert.Equal(t, columnNames, columns, "Columns() should return correct column names")
	...
	err = rs.Next(dest)
	assert.Equal(t, io.EOF, err, "Next() should return io.EOF when exhausted")
...
// (Tasks 2.15-2.18)
func TestPhaseB_E2E_ResultSetAcrossChunkBoundaries(t *testing.T) {
	// Create storage and catalog
	stor := storage.NewStorage()
	cat := catalog.NewCatalog()
	_ = cat // catalog used implicitly by executor
	...
	intInfo, _ := dukdb.NewTypeInfo(dukdb.TYPE_INTEGER)
	col1 := catalog.NewColumnDef("value", dukdb.TYPE_INTEGER)
	col1.TypeInfo = intInfo
	tableDef := catalog.NewTableDef("numbers", []*catalog.ColumnDef{col1})
	...
	require.GreaterOrEqual(t, len(chunks), 2, "Should have multiple chunks for this test")
	...
	for {
		...
		assert.Equal(t, int64(rowCount), dest[0], "Row %d should have correct value", rowCount)
		rowCount++
	}

	// Verify we got all rows
	assert.Equal(t, numRows, rowCount, "Should iterate through all rows across chunk boundaries")
...
// (Tasks 2.19-2.20)
func TestPhaseB_E2E_ResultSetTypeConversion(t *testing.T) {
	// Create storage and catalog
	stor := storage.NewStorage()
	cat := catalog.NewCatalog()
	_ = cat // catalog used implicitly by executor
	...
	table, err := stor.CreateTable("types_table", columnTypes)
	require.NoError(t, err)
	...
	tinyIntInfo, _ := dukdb.NewTypeInfo(dukdb.TYPE_TINYINT)
	smallIntInfo, _ := dukdb.NewTypeInfo(dukdb.TYPE_SMALLINT)
	intInfo, _ := dukdb.NewTypeInfo(dukdb.TYPE_INTEGER)
	bigIntInfo, _ := dukdb.NewTypeInfo(dukdb.TYPE_BIGINT)
	floatInfo, _ := dukdb.NewTypeInfo(dukdb.TYPE_FLOAT)
	doubleInfo, _ := dukdb.NewTypeInfo(dukdb.TYPE_DOUBLE)
	varcharInfo, _ := dukdb.NewTypeInfo(dukdb.TYPE_VARCHAR)

	columns := []*catalog.ColumnDef{
		{Name: "bool_col", Type: dukdb.TYPE_BOOLEAN, TypeInfo: boolInfo},
		{Name: "tiny_col", Type: dukdb.TYPE_TINYINT, TypeInfo: tinyIntInfo},
		{Name: "small_col", Type: dukdb.TYPE_SMALLINT, TypeInfo: smallIntInfo},
		{Name: "int_col", Type: dukdb.TYPE_INTEGER, TypeInfo: intInfo},
		{Name: "big_col", Type: dukdb.TYPE_BIGINT, TypeInfo: bigIntInfo},
		{Name: "float_col", Type: dukdb.TYPE_FLOAT, TypeInfo: floatInfo},
		{Name: "double_col", Type: dukdb.TYPE_DOUBLE, TypeInfo: doubleInfo},
		{Name: "varchar_col", Type: dukdb.TYPE_VARCHAR, TypeInfo: varcharInfo},
	}
	tableDef := catalog.NewTableDef("types_table", columns)
	...
	// Create ResultSet
	columnNames := []string{"bool_col", "tiny_col", "small_col", "int_col", "big_col", "float_col", "double_col", "varchar_col"}
	typeInfos := scanOp.GetTypes()
	rs := NewResultSet([]*storage.DataChunk{chunk}, typeInfos, columnNames)
	...
	assert.Equal(t, true, dest[0], "BOOLEAN should be bool")
	assert.Equal(t, int64(10), dest[1], "TINYINT should be int64")
	assert.Equal(t, int64(1000), dest[2], "SMALLINT should be int64")
	assert.Equal(t, int64(100000), dest[3], "INTEGER should be int64")
	assert.Equal(t, int64(10000000), dest[4], "BIGINT should be int64")
	assert.InDelta(t, 3.14, dest[5], 0.001, "FLOAT should be float64")
	assert.InDelta(t, 2.71828, dest[6], 0.00001, "DOUBLE should be float64")
	assert.Equal(t, "test string", dest[7], "VARCHAR should be string")
...
// (Tasks 2.21-2.22)
func TestPhaseB_E2E_ResultSetNullHandling(t *testing.T) {
	// Create storage and catalog
	stor := storage.NewStorage()
	cat := catalog.NewCatalog()
	_ = cat // catalog used implicitly by executor
	...
	table, err := stor.CreateTable("nullable_table", columnTypes)
	require.NoError(t, err)
	...
	columns := []*catalog.ColumnDef{
		{Name: "id", Type: dukdb.TYPE_INTEGER, TypeInfo: intInfo},
		{Name: "name", Type: dukdb.TYPE_VARCHAR, TypeInfo: varcharInfo},
		{Name: "salary", Type: dukdb.TYPE_DOUBLE, TypeInfo: doubleInfo},
	}
	tableDef := catalog.NewTableDef("nullable_table", columns)
	...
	require.NoError(t, err)
	assert.Equal(t, int64(2), dest[0])
	assert.Nil(t, dest[1], "NULL name should be nil")
	assert.Equal(t, float64(60000.0), dest[2])
	...
	assert.Equal(t, int64(3), dest[0])
	assert.Equal(t, "Charlie", dest[1])
	assert.Nil(t, dest[2], "NULL salary should be nil")

	// Row 4: NULL id
	err = rs.Next(dest)
	require.NoError(t, err)
	assert.Nil(t, dest[0], "NULL id should be nil")
	...
	err = rs.Next(dest)
	require.NoError(t, err)
	assert.Nil(t, dest[0], "All values should be nil")
	assert.Nil(t, dest[1], "All values should be nil")
	assert.Nil(t, dest[2], "All values should be nil")
...
// (Task 2.26)
func TestPhaseB_E2E_ResultSetColumnMetadata(t *testing.T) {
	// Create storage and catalog
	stor := storage.NewStorage()
	cat := catalog.NewCatalog()
	_ = cat // catalog used implicitly by executor
	...
	// Insert one row
	require.NoError(t, table.AppendRow([]any{int32(1), "Widget", int64(1250)})) // 12.50 as decimal
	...
	columns := []*catalog.ColumnDef{
		{Name: "id", Type: dukdb.TYPE_INTEGER, TypeInfo: intInfo},
		{Name: "name", Type: dukdb.TYPE_VARCHAR, TypeInfo: varcharInfo},
		{Name: "price", Type: dukdb.TYPE_DECIMAL, TypeInfo: decimalInfo},
	}
	...
	assert.Equal(t, "INTEGER", rs.ColumnTypeDatabaseTypeName(0))
	assert.Equal(t, "VARCHAR", rs.ColumnTypeDatabaseTypeName(1))
	assert.Equal(t, "DECIMAL(10,2)", rs.ColumnTypeDatabaseTypeName(2))

	// Test out of bounds
	assert.Equal(t, "", rs.ColumnTypeDatabaseTypeName(-1))
	assert.Equal(t, "", rs.ColumnTypeDatabaseTypeName(3))

	// Test ColumnTypeInfo
	typeInfo0 := rs.ColumnTypeInfo(0)
	require.NotNil(t, typeInfo0)
	assert.Equal(t, dukdb.TYPE_INTEGER, typeInfo0.InternalType())

	typeInfo2 := rs.ColumnTypeInfo(2)
	require.NotNil(t, typeInfo2)
	assert.Equal(t, dukdb.TYPE_DECIMAL, typeInfo2.InternalType())
	details := typeInfo2.Details()
	require.NotNil(t, details)
	decimalDetails, ok := details.(*dukdb.DecimalDetails)
	require.True(t, ok)
	assert.Equal(t, uint8(10), decimalDetails.Width)
	assert.Equal(t, uint8(2), decimalDetails.Scale)
...
func TestPhaseB_E2E_CompleteQueryPipeline(t *testing.T) {
	// Create storage and catalog
	stor := storage.NewStorage()
	cat := catalog.NewCatalog()
	...
	for _, row := range testData {
		require.NoError(t, table.AppendRow([]any{row.id, row.name, row.salary, row.active}))
	}
	...
	columns := []*catalog.ColumnDef{
		...
		{Name: "name", Type: dukdb.TYPE_VARCHAR, TypeInfo: varcharInfo},
		{Name: "salary", Type: dukdb.TYPE_DOUBLE, TypeInfo: doubleInfo},
		{Name: "active", Type: dukdb.TYPE_BOOLEAN, TypeInfo: boolInfo},
	}
	tableDef := catalog.NewTableDef("employees", columns)
	...
	assert.Equal(t, "VARCHAR", rs.ColumnTypeDatabaseTypeName(0))
	assert.Equal(t, "DOUBLE", rs.ColumnTypeDatabaseTypeName(1))
	...
	for {
		...
		require.Less(t, rowCount, len(expectedResults), "More rows than expected")

		// Check if this row matches any expected result (order may vary)
		name, ok := dest[0].(string)
		require.True(t, ok, "dest[0] should be a string")
		salary, ok := dest[1].(float64)
		require.True(t, ok, "dest[1] should be a float64")

		found := false
		for _, expected := range expectedResults {
			if expected.name == name && expected.salary == salary {
				found = true

				break
			}
		}
		assert.True(t, found, "Row (%s, %f) should match an expected result", name, salary)

		rowCount++
	}

	// Verify we got the expected number of rows
	assert.Equal(t, len(expectedResults), rowCount, "Should have correct number of result rows")
...
func TestPhaseB_E2E_EmptyResultSet(t *testing.T) {
	...
	tableDef := catalog.NewTableDef("empty_table", columns)
	...
	dest := make([]driver.Value, 2)
	err = rs.Next(dest)
	assert.Equal(t, io.EOF, err, "Empty result set should return EOF immediately")
...
func TestPhaseB_E2E_ResultSetWithFilteredData(t *testing.T) {
	// Create storage and catalog
	stor := storage.NewStorage()
	cat := catalog.NewCatalog()
	exec := NewExecutor(cat, stor)
	...
	intInfo, _ := dukdb.NewTypeInfo(dukdb.TYPE_INTEGER)
	columns := []*catalog.ColumnDef{
		{Name: "value", Type: dukdb.TYPE_INTEGER, TypeInfo: intInfo},
	}
	tableDef := catalog.NewTableDef("numbers", columns)
	...
	filterPredicate := &binder.BoundBinaryExpr{
		Op:      parser.OpEq,
		Left:    &binder.BoundColumnRef{Column: "value", ColType: dukdb.TYPE_INTEGER},
		Right:   &binder.BoundLiteral{Value: int32(42), ValType: dukdb.TYPE_INTEGER},
		ResType: dukdb.TYPE_BOOLEAN,
	}

	childColumns := []planner.ColumnBinding{
		{Table: "numbers", Column: "value", Type: dukdb.TYPE_INTEGER},
	}
