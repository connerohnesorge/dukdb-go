...
{"id": 5, "name": "Eve", "age": 32, "city": "Seattle", "timestamp": "2024-01-15T14:00:00Z"}`
	...
	for rows.Next() {
		...
		fmt.Printf("ID: %d, Name: %s, Age: %d, City: %s, Time: %s\n", id, name, age, city, timestamp)
	}
	...
	query = fmt.Sprintf("SELECT name, city, timestamp FROM read_ndjson('%s') WHERE timestamp \u003e '2024-01-15T11:30:00Z' ORDER BY timestamp", sampleFile)
	rows, err = db.Query(query)
	if err != nil {
	...
	fmt.Println("\n=== Example 4: Aggregating NDJSON Data ===")
	query = fmt.Sprintf("SELECT city, COUNT(*), AVG(age) FROM read_ndjson('%s') GROUP BY city ORDER BY COUNT(*) DESC", sampleFile)
	rows, err = db.Query(query)
	if err != nil {
	...
	// Write 1000 records
	for i := 0; i < 1000; i++ {
		record := fmt.Sprintf(`{"id": %d, "event_type": "click", "user_id": %d, "timestamp": "2024-01-15T%02d:%02d:%02dZ"}`,
			i+1, (i%100)+1, (i/60)%24, i%60, i%60)
...
{"id": 3, "name": "Charlie"}`
	...
	query = fmt.Sprintf("SELECT COUNT(DISTINCT city) as unique_cities FROM read_ndjson('%s')", absPath)
	rows, err = db.Query(query)
	if err != nil {
