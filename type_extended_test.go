...
func TestUhugeint(t *testing.T) {
	...
	t.Run("NewUhugeint simple value", func(t *testing.T) {
		u, err := NewUhugeint(big.NewInt(12345))
		require.NoError(t, err)
		assert.Equal(t, uint64(12345), u.Lower())
		assert.Equal(t, uint64(0), u.Upper())
		assert.Equal(t, "12345", u.String())
	})

	t.Run("NewUhugeint max uint64", func(t *testing.T) {
		bi := new(big.Int).SetUint64(^uint64(0))
	...
	t.Run("NewUhugeint large value", func(t *testing.T) {
		// Value larger than uint64 max.
		bi, _ := new(big.Int).SetString("18446744073709551616", 10) // 2^64
		u, err := NewUhugeint(bi)
		require.NoError(t, err)
		assert.Equal(t, uint64(0), u.Lower())
	...
	t.Run("NewUhugeint max value", func(t *testing.T) {
		bi, _ := new(big.Int).SetString(UhugeintMaxString, 10)
		u, err := NewUhugeint(bi)
		require.NoError(t, err)
		assert.Equal(t, ^uint64(0), u.Lower())
		assert.Equal(t, ^uint64(0), u.Upper())
	...
	t.Run("NewUhugeint overflow", func(t *testing.T) {
		// Value exceeding 128 bits.
		bi, _ := new(big.Int).SetString("340282366920938463463374607431768211456", 10) // Max + 1
		_, err := NewUhugeint(bi)
		assert.Error(t, err)
		assert.Contains(t, err.Error(), "exceeds UHUGEINT maximum")
	})

	t.Run("NewUhugeint negative", func(t *testing.T) {
		_, err := NewUhugeint(big.NewInt(-1))
		assert.Error(t, err)
		assert.Contains(t, err.Error(), "cannot be negative")
	})

	t.Run("ToBigInt roundtrip", func(t *testing.T) {
		original, _ := new(big.Int).SetString("123456789012345678901234567890", 10)
	...
	t.Run("NewUhugeintFromParts", func(t *testing.T) {
		u := NewUhugeintFromParts(1, 2)
		assert.Equal(t, uint64(2), u.Lower())
		assert.Equal(t, uint64(1), u.Upper())
	})

	t.Run("NewUhugeintFromUint64", func(t *testing.T) {
	...
	t.Run("Scan string", func(t *testing.T) {
		var u Uhugeint
		err := u.Scan("123456789012345678901234567890")
		require.NoError(t, err)
		assert.Equal(t, "123456789012345678901234567890", u.String())
	})
...
func TestBit(t *testing.T) {
	...
	t.Run("NewBit invalid character", func(t *testing.T) {
		_, err := NewBit("1012")
		assert.Error(t, err)
		assert.Contains(t, err.Error(), "invalid bit character")
	})

	t.Run("NewBitFromBytes", func(t *testing.T) {
		b := NewBitFromBytes([]byte{0b10110000}, 5)
		assert.Equal(t, 5, b.Len())
		assert.Equal(t, "10110", b.String())
	})
	...
	t.Run("And length mismatch", func(t *testing.T) {
		a, _ := NewBit("10110")
		b, _ := NewBit("1101")
		_, err := a.And(b)
		assert.Error(t, err)
...
func TestTimeNS(t *testing.T) {
	...
	t.Run("String without nanos", func(t *testing.T) {
		tns := NewTimeNS(14, 30, 45, 0)
		assert.Equal(t, "14:30:45", tns.String())
	})

	t.Run("String with nanos", func(t *testing.T) {
		tns := NewTimeNS(14, 30, 45, 123456789)
		assert.Equal(t, "14:30:45.123456789", tns.String())
	})

	t.Run("String with leading zero nanos", func(t *testing.T) {
		tns := NewTimeNS(14, 30, 45, 1)
		assert.Equal(t, "14:30:45.000000001", tns.String())
	})
	...
	t.Run("Scan int64", func(t *testing.T) {
		...
		nanos := int64(14)*nanosPerHour + int64(30)*nanosPerMinute + int64(45)*nanosPerSecond + 123456789
		err := tns.Scan(nanos)
		require.NoError(t, err)
		h, m, s, ns := tns.Components()
		assert.Equal(t, 14, h)
	...
	t.Run("Scan string no nanos", func(t *testing.T) {
		var tns TimeNS
	...
	t.Run("Scan string partial nanos", func(t *testing.T) {
		var tns TimeNS
...
func TestTimeNS_EdgeCases(t *testing.T) {
	...
	t.Run("nanosecond precision roundtrip", func(t *testing.T) {
		for _, ns := range []int64{1, 123, 123456, 123456789, 999999999} {
			tns := NewTimeNS(12, 30, 45, ns)
			_, _, _, gotNS := tns.Components()
			assert.Equal(t, ns, gotNS, "nanosecond precision lost for %d", ns)
		}
	})
}
...
func TestCurrentTimeNS(t *testing.T) {
	t.Run("basic mock", func(t *testing.T) {
		...
		// Set a specific time.
		fixedTime := time.Date(2024, 6, 15, 14, 30, 45, 123456789, time.UTC)
	...
	t.Run("nanosecond precision", func(t *testing.T) {
		...
		for _, tc := range testCases {
			fixedTime := time.Date(2024, 6, 15, 12, 0, 0, tc.ns, time.UTC)
			mockClock.Set(fixedTime)

			tns := CurrentTimeNS(mockClock)
			_, _, _, gotNS := tns.Components()
			assert.Equal(t, tc.expected, gotNS)
		}
	})
	...
	t.Run("deterministic behavior", func(t *testing.T) {
		...
		assert.Equal(t, "14:30:45.123456789", tns1.String())
	})
}
