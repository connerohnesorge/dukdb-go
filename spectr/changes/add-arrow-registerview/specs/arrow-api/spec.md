...
- **data-chunk-api**: Requires DataChunk to Arrow conversion (reverse of existing Arrow to DataChunk)
- **deterministic-testing** (`spectr/specs/deterministic-testing/spec.md`): All tests must use quartz.Clock, no time.Sleep
- **catalog-api**: Requires virtual table registration support in catalog
- **query-execution**: Requires virtual table scan integration in execution engine
