# Spec: Cost-Based Query Optimizer

## Overview

Cost-based query optimization for dukdb-go that uses statistics to select optimal join orders, access methods, and execution strategies.
...
#### Test: TPC-H Query 3 Optimization
```sql
...
LIMIT 10;
```
- **Expected**: Optimizer selects efficient join order
- **Expected**: Customer filtered first (small result)
- **Expected**: Plan cost reflects actual performance

#### Test: Join order improvement
```sql
-- Without optimization: large x large x small
SELECT * FROM large1, large2, small WHERE ...
-- With optimization: small x large1 x large2
```
- **Expected**: Optimizer reorders to minimize intermediates

#### Test: Statistics accuracy
- Create table with known distribution
- Run ANALYZE
- Verify statistics match actual data within tolerance

#### Test: Cost model calibration
- Run queries with different plans
- Verify lower cost plans execute faster
