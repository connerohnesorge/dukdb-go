# Spec: Parallel Query Execution

## Overview

Parallel query execution for dukdb-go using Go's goroutine-based concurrency to enable multi-threaded execution of table scans, hash joins, aggregations, and other operators.
...
#### Test: Parallel scan matches sequential
```sql
SELECT * FROM large_table WHERE x > 100;
```
- **Expected**: Same rows in same order as sequential

#### Test: Parallel hash join correctness
```sql
SELECT * FROM orders o JOIN customers c ON o.customer_id = c.id;
```
- **Expected**: All matching rows, outer join semantics preserved

#### Test: Parallel aggregation correctness
```sql
SELECT department, SUM(salary), COUNT(*) FROM employees GROUP BY department;
```
- **Expected**: Correct aggregates per group

### Performance Tests

#### Test: Linear speedup on analytical query
- Execute TPC-H Q1 with 1, 2, 4, 8 workers
- **Expected**: Near-linear speedup up to core count

#### Test: No regression on small queries
- Execute simple single-row lookup
- **Expected**: No measurable overhead from parallel infrastructure
