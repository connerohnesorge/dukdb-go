...
**Created:** 2026-03-20
**Scope:** Small — Adds 15 small utility functions across 4 categories
**Estimated Complexity:** Low — Each function is 5-20 lines, self-contained
**User-Visible:** Yes — New SQL functions available

## Summary

This proposal bundles 15 small missing utility functions into a single change:

1. **System:** `CURRENT_DATABASE()`, `CURRENT_SCHEMA()`, `VERSION()`
2. **Date/Time:** `DAYNAME(date)`, `MONTHNAME(date)`, `YEARWEEK(date)`, `EPOCH_US(timestamp)`, `NOW()`, `CURRENT_TIMESTAMP`, `CURRENT_DATE`, `CURRENT_TIME`
3. **String:** `TRANSLATE(str, from, to)`, `STRIP_ACCENTS(str)`
4. **NULL handling:** `IFNULL(expr, default)`, `NVL(expr, default)`
...
- `golang.org/x/text` v0.34.0 already in `go.mod` — needed for `STRIP_ACCENTS`
...
1. Register all 15 functions following the existing dispatch pattern
