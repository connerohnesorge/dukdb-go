...
```go
// StreamingResult delivers query results one row at a time via scanNext.
// Close() MUST be called to release executor resources.
// NOTE: This struct already exists in streaming.go with these exact fields.
...
The `rows` struct in `conn.go` (line ~383) gains a streaming mode:
...
type BackendConnStreaming interface {
    ...
    QueryStreaming(
        ...
        query string,
        args []driver.NamedValue,
    ) (*StreamingResult, []string, error) // returns result, column names, error
}
```
