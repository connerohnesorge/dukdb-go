# Spec: DuckDB Storage Format

## Overview

Native DuckDB file format (.duckdb) support for reading and writing databases compatible with DuckDB v1.4.3 CLI and tools.
...
#### Test: Read simple table from DuckDB
```sql
-- In DuckDB CLI
CREATE TABLE test (id INTEGER, name VARCHAR);
INSERT INTO test VALUES (1, 'Alice'), (2, 'Bob');
```
- **Expected**: dukdb-go reads both rows correctly

#### Test: Write table readable by DuckDB
```go
// In dukdb-go
db.Exec("CREATE TABLE test (id INTEGER, name VARCHAR)")
db.Exec("INSERT INTO test VALUES (1, 'Alice')")
db.Close()
```
- **Expected**: DuckDB CLI reads the table correctly

#### Test: All data types round-trip
- Create table with all 43 supported types
- Insert representative values
- Close and reopen
- Verify all values preserved

#### Test: Large file handling
- Create table with 10 million rows
- Verify row groups are created
