...
- **data-chunk-api**: Requires DataChunk.SetValue() for supported types (excluding INVALID, UHUGEINT, BIT, ANY, BIGNUM per duckdb-go/type.go:52-58)
- **deterministic-testing** (`spectr/specs/deterministic-testing/spec.md`): All tests must use quartz.Clock, no time.Sleep
- **transaction-management**: Requires LocalStorage pattern for transaction-local row groups
- **storage-layer**: Requires row group management and chunk appending (not via Backend interface)
