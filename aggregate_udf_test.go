...
func TestAggregateFuncConfig(t *testing.T) {
	...
	assert.Equal(
		t,
		TYPE_INTEGER,
		config.InputTypeInfos[0].InternalType(),
	)
	assert.Equal(
		t,
		TYPE_BIGINT,
		config.ResultTypeInfo.InternalType(),
	)
...
func TestAggregateRegistryValidation(
	t *testing.T,
) {
	...
	assert.Contains(
		t,
		err.Error(),
		"must have either Update or UpdateCtx",
	)
	...
	assert.Contains(
		t,
		err.Error(),
		"must have either Finalize or FinalizeCtx",
	)
	...
	nilInputFunc.Config.InputTypeInfos = []TypeInfo{
		nil,
	}
	...
	assert.Contains(
		t,
		err.Error(),
		"input type at index 0 cannot be nil",
	)
...
func TestAggregateRegistryLookup(t *testing.T) {
	...
	assert.Equal(
		t,
		TYPE_BIGINT,
		fn.config.ResultTypeInfo.InternalType(),
	)
	...
	udfInfo, resultType, found := registry.LookupAggregateUDF(
		"my_sum",
		[]Type{TYPE_INTEGER},
	)
	assert.True(t, found)
...
func TestAggregateExecutionStateBasic(
	t *testing.T,
) {
	intType, _ := NewTypeInfo(TYPE_INTEGER)
	bigintType, _ := NewTypeInfo(TYPE_BIGINT)
	...
	execState := NewAggregateExecutionState(
		reg,
		context.Background(),
		mClock,
	)
...
func TestAggregateExecutionStateCombine(
	t *testing.T,
) {
	intType, _ := NewTypeInfo(TYPE_INTEGER)
	bigintType, _ := NewTypeInfo(TYPE_BIGINT)
	...
	state1 := NewAggregateExecutionState(
		reg,
		context.Background(),
		mClock,
	)
	state2 := NewAggregateExecutionState(
		reg,
		context.Background(),
		mClock,
	)
	...
	// Process chunks
	_ = state1.ProcessChunk(
		chunk1,
		[]int{},
		[]int{0},
	)
	_ = state2.ProcessChunk(
		chunk2,
		[]int{},
		[]int{0},
	)
...
func TestAggregateExecutionStateNullHandling(
	t *testing.T,
) {
	intType, _ := NewTypeInfo(TYPE_INTEGER)
	bigintType, _ := NewTypeInfo(TYPE_BIGINT)
	...
	execState2 := NewAggregateExecutionState(
		&regWithNull,
		context.Background(),
		nil,
	)
	...
	assert.Equal(
		...
		int64(3),
		results2[0].Value,
	) // All values counted including NULL
}

// Deterministic Tests with Mock Clock

func TestAggregateFuncContextWithClock(
	t *testing.T,
) {
	mClock := quartz.NewMock(t)
	mClock.Set(
...
func TestAggregateFuncContextGroupKey(
	t *testing.T,
) {
	mClock := quartz.NewMock(t)
...
func TestAggregateFuncContextDefaultClock(
	t *testing.T,
) {
...
func TestAggregateUDFPanicRecoveryUpdate(
	t *testing.T,
) {
	...
	assert.Contains(
		t,
		err.Error(),
		"panic in aggregate UDF Update",
	)
	assert.Contains(
		t,
		err.Error(),
		"intentional panic in Update",
	)
}

func TestAggregateUDFPanicRecoveryUpdateContext(
	t *testing.T,
) {
...
func TestAggregateUDFPanicRecoveryCombine(
	t *testing.T,
) {
	...
	assert.Contains(
		t,
		err.Error(),
		"panic in aggregate UDF Combine",
	)
}

func TestAggregateUDFPanicRecoveryFinalize(
	t *testing.T,
) {
	...
	_, err := safeAggregateFinalizeSimple(
		panicFinalize,
		nil,
	)
	require.Error(t, err)
	assert.Contains(
		t,
		err.Error(),
		"panic in aggregate UDF Finalize",
	)
}

func TestAggregateUDFPanicRecoveryFinalizeContext(
	t *testing.T,
) {
...
func TestAggregateUDFPanicRecoveryDestroy(
	t *testing.T,
) {
...
func TestAggregateFuncContextTimeout(
	t *testing.T,
) {
	mClock := quartz.NewMock(t)
	mClock.Set(
	...
	deadline := time.Now().
		Add(1 * time.Hour)
		// Real time deadline in the future
	...
	pastDeadline := mClock.Now().
		Add(-100 * time.Millisecond)
	ctx2, cancel2 := context.WithDeadline(
		context.Background(),
		pastDeadline,
	)
	defer cancel2()
...
func TestAggregateFuncContextCancellation(
	t *testing.T,
) {
...
func TestMatchesTypesForAggregate(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := matchesTypesForAggregate(
				tt.inputTypes,
				tt.argTypes,
				tt.variadicType,
			)
...
func TestAllCallbackTypesWork(t *testing.T) {
	...
	exec := NewAggregateExecutionState(
		fn,
		context.Background(),
		nil,
	)
