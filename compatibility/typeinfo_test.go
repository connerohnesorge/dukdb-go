...
func TestTypeInfoAPICompatibility(t *testing.T) {
	t.Run("TypeInfo interface methods", func(t *testing.T) {
		info, err := dukdb.NewTypeInfo(dukdb.TYPE_INTEGER)
		require.NoError(t, err)

		// Verify interface methods exist and work
		typeInfo := info
		assert.Equal(t, dukdb.TYPE_INTEGER, typeInfo.InternalType())
		assert.Nil(t, typeInfo.Details())
		assert.Equal(t, "INTEGER", typeInfo.SQLType())
	})

	t.Run("NewTypeInfo for all primitive types", func(t *testing.T) {
		primitives := []struct {
			typ     dukdb.Type
			sqlType string
		}{
			...
			{dukdb.TYPE_TIMESTAMP_S, "TIMESTAMP_S"},
			{dukdb.TYPE_TIMESTAMP_MS, "TIMESTAMP_MS"},
			{dukdb.TYPE_TIMESTAMP_NS, "TIMESTAMP_NS"},
			{dukdb.TYPE_TIMESTAMP_TZ, "TIMESTAMPTZ"},
			{dukdb.TYPE_INTERVAL, "INTERVAL"},
		}

		for _, tc := range primitives {
			info, err := dukdb.NewTypeInfo(tc.typ)
			require.NoError(t, err, "NewTypeInfo(%v)", tc.typ)
			assert.Equal(t, tc.typ, info.InternalType())
	...
	t.Run("NewTypeInfo rejects complex types", func(t *testing.T) {
		...
		for _, typ := range complexTypes {
			...
			assert.Error(t, err, "NewTypeInfo(%v) should error", typ)
		}
	})

	t.Run("NewDecimalInfo API", func(t *testing.T) {
		info, err := dukdb.NewDecimalInfo(10, 2)
		require.NoError(t, err)

		assert.Equal(t, dukdb.TYPE_DECIMAL, info.InternalType())
		assert.Equal(t, "DECIMAL(10,2)", info.SQLType())

		details, ok := info.Details().(*dukdb.DecimalDetails)
		require.True(t, ok, "Details should be *DecimalDetails")
		assert.Equal(t, uint8(10), details.Width)
		assert.Equal(t, uint8(2), details.Scale)
	})
	...
	t.Run("NewEnumInfo API", func(t *testing.T) {
		...
		assert.Equal(t, dukdb.TYPE_ENUM, info.InternalType())
		assert.Equal(t, "ENUM('small', 'medium', 'large')", info.SQLType())

		details, ok := info.Details().(*dukdb.EnumDetails)
		require.True(t, ok, "Details should be *EnumDetails")
	...
	t.Run("NewListInfo API", func(t *testing.T) {
		...
		assert.Equal(t, dukdb.TYPE_LIST, info.InternalType())
		assert.Equal(t, "INTEGER[]", info.SQLType())

		details, ok := info.Details().(*dukdb.ListDetails)
		require.True(t, ok, "Details should be *ListDetails")
		assert.Equal(t, dukdb.TYPE_INTEGER, details.Child.InternalType())
	})

	t.Run("NewArrayInfo API", func(t *testing.T) {
		childInfo, _ := dukdb.NewTypeInfo(dukdb.TYPE_INTEGER)
		info, err := dukdb.NewArrayInfo(childInfo, 5)
		require.NoError(t, err)

		assert.Equal(t, dukdb.TYPE_ARRAY, info.InternalType())
		assert.Equal(t, "INTEGER[5]", info.SQLType())

		details, ok := info.Details().(*dukdb.ArrayDetails)
		require.True(t, ok, "Details should be *ArrayDetails")
		assert.Equal(t, dukdb.TYPE_INTEGER, details.Child.InternalType())
	...
	t.Run("NewMapInfo API", func(t *testing.T) {
		...
		info, err := dukdb.NewMapInfo(keyInfo, valueInfo)
		require.NoError(t, err)

		assert.Equal(t, dukdb.TYPE_MAP, info.InternalType())
		assert.Equal(t, "MAP(VARCHAR, INTEGER)", info.SQLType())

		details, ok := info.Details().(*dukdb.MapDetails)
		require.True(t, ok, "Details should be *MapDetails")
		assert.Equal(t, dukdb.TYPE_VARCHAR, details.Key.InternalType())
		assert.Equal(t, dukdb.TYPE_INTEGER, details.Value.InternalType())
	})

	t.Run("NewStructEntry and NewStructInfo API", func(t *testing.T) {
		intInfo, _ := dukdb.NewTypeInfo(dukdb.TYPE_INTEGER)
		strInfo, _ := dukdb.NewTypeInfo(dukdb.TYPE_VARCHAR)

		entry1, err := dukdb.NewStructEntry(intInfo, "id")
		require.NoError(t, err)
		assert.Equal(t, "id", entry1.Name())
		assert.Equal(t, dukdb.TYPE_INTEGER, entry1.Info().InternalType())

		entry2, _ := dukdb.NewStructEntry(strInfo, "name")

		info, err := dukdb.NewStructInfo(entry1, entry2)
		require.NoError(t, err)

		assert.Equal(t, dukdb.TYPE_STRUCT, info.InternalType())
		assert.Equal(t, `STRUCT("id" INTEGER, "name" VARCHAR)`, info.SQLType())

		details, ok := info.Details().(*dukdb.StructDetails)
		require.True(t, ok, "Details should be *StructDetails")
	...
	t.Run("NewUnionInfo API", func(t *testing.T) {
		...
		assert.Equal(t, dukdb.TYPE_UNION, info.InternalType())
		assert.Equal(t, `UNION("num" INTEGER, "str" VARCHAR)`, info.SQLType())

		details, ok := info.Details().(*dukdb.UnionDetails)
		require.True(t, ok, "Details should be *UnionDetails")
		require.Len(t, details.Members, 2)
		assert.Equal(t, "num", details.Members[0].Name)
		assert.Equal(t, dukdb.TYPE_INTEGER, details.Members[0].Type.InternalType())
		assert.Equal(t, "str", details.Members[1].Name)
		assert.Equal(t, dukdb.TYPE_VARCHAR, details.Members[1].Type.InternalType())
	})
}

// TestTypeDetailsAPICompatibility verifies that TypeDetails implementations
// expose the expected fields.
func TestTypeDetailsAPICompatibility(t *testing.T) {
	t.Run("DecimalDetails fields", func(t *testing.T) {
	...
	t.Run("EnumDetails fields", func(t *testing.T) {
	...
	t.Run("ListDetails fields", func(t *testing.T) {
		intInfo, _ := dukdb.NewTypeInfo(dukdb.TYPE_INTEGER)
	...
	t.Run("ArrayDetails fields", func(t *testing.T) {
		intInfo, _ := dukdb.NewTypeInfo(dukdb.TYPE_INTEGER)
		info, _ := dukdb.NewArrayInfo(intInfo, 10)
	...
	t.Run("MapDetails fields", func(t *testing.T) {
		keyInfo, _ := dukdb.NewTypeInfo(dukdb.TYPE_VARCHAR)
		valueInfo, _ := dukdb.NewTypeInfo(dukdb.TYPE_INTEGER)
	...
	t.Run("StructDetails fields", func(t *testing.T) {
		intInfo, _ := dukdb.NewTypeInfo(dukdb.TYPE_INTEGER)
		entry, _ := dukdb.NewStructEntry(intInfo, "field")
	...
	t.Run("UnionDetails and UnionMember fields", func(t *testing.T) {
		intInfo, _ := dukdb.NewTypeInfo(dukdb.TYPE_INTEGER)
		info, _ := dukdb.NewUnionInfo([]dukdb.TypeInfo{intInfo}, []string{"a"})
...
func TestStructEntryAPICompatibility(t *testing.T) {
	intInfo, _ := dukdb.NewTypeInfo(dukdb.TYPE_INTEGER)
	entry, err := dukdb.NewStructEntry(intInfo, "test_field")
	require.NoError(t, err)

	// Verify interface methods
	structEntry := entry
	assert.Equal(t, "test_field", structEntry.Name())
	assert.Equal(t, dukdb.TYPE_INTEGER, structEntry.Info().InternalType())
}

// TestTypeInfoCacheAPICompatibility verifies the cache helper functions.
func TestTypeInfoCacheAPICompatibility(t *testing.T) {
	...
	_, err := dukdb.NewTypeInfo(dukdb.TYPE_INTEGER)
	require.NoError(t, err)
