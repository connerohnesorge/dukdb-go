...
var SQLCompatibilityTests = []CompatibilityTest{
	...
	{
		Name:     "SelectJoinInner",
		Category: "sql",
		Test:     testSelectJoinInner,
	},
	{
		Name:     "SelectJoinLeft",
		Category: "sql",
		Test:     testSelectJoinLeft,
	},
}
...
func testCreateTable(t *testing.T, db *sql.DB) {
	_, err := db.Exec(
		`CREATE TABLE test_create (id INTEGER, name VARCHAR)`,
	)
	require.NoError(t, err)

	// Verify table exists by inserting and querying
	_, err = db.Exec(
		`INSERT INTO test_create VALUES (1, 'test')`,
	)
	require.NoError(t, err)

	var id int
	var name string
	err = db.QueryRow(`SELECT id, name FROM test_create`).
		Scan(&id, &name)
	require.NoError(t, err)
	assert.Equal(t, 1, id)
...
func testCreateTableIfNotExists(
	...
	_, err := db.Exec(
		`CREATE TABLE test_if_not_exists (id INTEGER)`,
	)
	require.NoError(t, err)

	// Should not error when table already exists
	_, err = db.Exec(
		`CREATE TABLE IF NOT EXISTS test_if_not_exists (id INTEGER)`,
	)
	require.NoError(t, err)
}
...
func testDropTable(t *testing.T, db *sql.DB) {
	_, err := db.Exec(
		`CREATE TABLE test_drop (id INTEGER)`,
	)
	require.NoError(t, err)
...
func testDropTableIfExists(
	...
	// Create and drop
	_, err = db.Exec(
		`CREATE TABLE test_drop_if (id INTEGER)`,
	)
	require.NoError(t, err)

	_, err = db.Exec(
		`DROP TABLE IF EXISTS test_drop_if`,
	)
	require.NoError(t, err)
}
...
func testSelectStar(t *testing.T, db *sql.DB) {
	_, err := db.Exec(
		`CREATE TABLE test_select_star (id INTEGER, name VARCHAR)`,
	)
	require.NoError(t, err)

	_, err = db.Exec(
		`INSERT INTO test_select_star VALUES (1, 'Alice'), (2, 'Bob')`,
	)
	require.NoError(t, err)
...
func testSelectColumns(t *testing.T, db *sql.DB) {
	_, err := db.Exec(
		`CREATE TABLE test_select_cols (id INTEGER, name VARCHAR, extra INTEGER)`,
	)
	require.NoError(t, err)

	_, err = db.Exec(
		`INSERT INTO test_select_cols VALUES (1, 'Alice', 100)`,
	)
	require.NoError(t, err)

	var id int
	var name string
	err = db.QueryRow(`SELECT id, name FROM test_select_cols`).
		Scan(&id, &name)
	require.NoError(t, err)
	assert.Equal(t, 1, id)
...
func testSelectWhere(t *testing.T, db *sql.DB) {
	_, err := db.Exec(
		`CREATE TABLE test_select_where (id INTEGER, name VARCHAR)`,
	)
	require.NoError(t, err)

	_, err = db.Exec(
		`INSERT INTO test_select_where VALUES (1, 'Alice'), (2, 'Bob'), (3, 'Charlie')`,
	)
	require.NoError(t, err)

	var name string
	err = db.QueryRow(`SELECT name FROM test_select_where WHERE id = 2`).
		Scan(&name)
	require.NoError(t, err)
...
func testSelectLimit(t *testing.T, db *sql.DB) {
	_, err := db.Exec(
		`CREATE TABLE test_limit (id INTEGER)`,
	)
	require.NoError(t, err)
...
func testSelectLimitOffset(
	...
	_, err := db.Exec(
		`CREATE TABLE test_limit_offset (id INTEGER)`,
	)
	require.NoError(t, err)

	_, err = db.Exec(
		`INSERT INTO test_limit_offset VALUES (1), (2), (3), (4), (5)`,
	)
	require.NoError(t, err)
...
func testSelectDistinct(
	...
	_, err := db.Exec(
		`CREATE TABLE test_distinct (category VARCHAR)`,
	)
	require.NoError(t, err)

	_, err = db.Exec(
		`INSERT INTO test_distinct VALUES ('A'), ('B'), ('A'), ('C'), ('B')`,
	)
	require.NoError(t, err)
...
func testSelectJoinInner(
	...
	_, err = db.Exec(
		`CREATE TABLE user_orders (user_id INTEGER, product VARCHAR)`,
	)
	require.NoError(t, err)
	...
	_, err = db.Exec(
		`INSERT INTO user_orders VALUES (1, 'Apple'), (1, 'Banana'), (2, 'Cherry')`,
	)
	require.NoError(t, err)
...
func testSelectJoinLeft(
	t *testing.T,
	db *sql.DB,
) {
	_, err := db.Exec(
		`CREATE TABLE left_users (id INTEGER, name VARCHAR)`,
	)
	require.NoError(t, err)

	_, err = db.Exec(
		`CREATE TABLE left_orders (user_id INTEGER, product VARCHAR)`,
	)
	require.NoError(t, err)

	_, err = db.Exec(
		`INSERT INTO left_users VALUES (1, 'Alice'), (2, 'Bob')`,
	)
	require.NoError(t, err)

	_, err = db.Exec(
		`INSERT INTO left_orders VALUES (1, 'Apple')`,
	)
	require.NoError(t, err)
