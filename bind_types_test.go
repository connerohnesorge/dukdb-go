...
func TestListValue(t *testing.T) {
	...
	t.Run("float64 slice", func(t *testing.T) {
		lv := NewListValue([]float64{1.1, 2.2, 3.3})
		val, err := lv.Value()
		require.NoError(t, err)
...
func TestStructValue(t *testing.T) {
	t.Run("simple struct", func(t *testing.T) {
		...
		sv := NewStructValue(Person{Name: "Alice", Age: 30})
		val, err := sv.Value()
		require.NoError(t, err)
	...
	t.Run("struct with custom tags", func(t *testing.T) {
		type Record struct {
			ID    int64  `duckdb:"record_id"`
			Value string `duckdb:"record_value"`
		}
		sv := NewStructValue(Record{ID: 42, Value: "test"})
		val, err := sv.Value()
		require.NoError(t, err)
		m, ok := val.(map[string]any)
		require.True(t, ok)
		assert.Equal(t, int64(42), m["record_id"])
		assert.Equal(t, "test", m["record_value"])
	})

	t.Run("struct without tags uses field name", func(t *testing.T) {
		type Simple struct {
			FieldOne string
			FieldTwo int
		}
		sv := NewStructValue(Simple{FieldOne: "value", FieldTwo: 123})
		val, err := sv.Value()
		require.NoError(t, err)
		m, ok := val.(map[string]any)
		require.True(t, ok)
		// Should use lowercase field names
		assert.Equal(t, "value", m["fieldone"])
...
func TestArrayValue(t *testing.T) {
	...
	t.Run("string array", func(t *testing.T) {
		av := NewArrayValue([]string{"a", "b", "c"})
		val, err := av.Value()
		require.NoError(t, err)
		assert.Equal(t, []any{"a", "b", "c"}, val)
	})

	t.Run("fixed size validation in usage", func(t *testing.T) {
...
func TestJSONValue(t *testing.T) {
	t.Run("struct value", func(t *testing.T) {
		...
		jv := NewJSONValue(Config{Enabled: true, Name: "test"})
		val, err := jv.Value()
		require.NoError(t, err)
