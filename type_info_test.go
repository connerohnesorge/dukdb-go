...
func TestErrTypeInfo(t *testing.T) {
	...
	validStructEntry, err := NewStructEntry(
		validInfo,
		"hello",
	)
	require.NoError(t, err)
	otherValidStructEntry, err := NewStructEntry(
		validInfo,
		"you",
	)
	require.NoError(t, err)
	nilStructEntry, err := NewStructEntry(
		nil,
		"hello",
	)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	unionStringInfo, err := NewTypeInfo(
		TYPE_VARCHAR,
	)
	require.NoError(t, err)
...
func TestTypeInfoDetails(t *testing.T) {
	...
	t.Run("ArrayDetails", func(t *testing.T) {
		...
		require.Equal(
			t,
			TYPE_VARCHAR,
			arrayDetails.Child.InternalType(),
		)
		require.Equal(
			t,
			uint64(5),
	...
	t.Run("MapDetails", func(t *testing.T) {
		...
		require.Equal(
			t,
			TYPE_INTEGER,
			mapDetails.Key.InternalType(),
		)
		require.Equal(
			t,
			TYPE_VARCHAR,
			mapDetails.Value.InternalType(),
		)
	})
	...
	t.Run("UnionDetails", func(t *testing.T) {
		...
		require.Equal(
			t,
			"num",
			unionDetails.Members[0].Name,
		)
		require.Equal(
			t,
			TYPE_INTEGER,
			unionDetails.Members[0].Type.InternalType(),
		)
		require.Equal(
			t,
			"text",
			unionDetails.Members[1].Name,
		)
		require.Equal(
			t,
			TYPE_VARCHAR,
			unionDetails.Members[1].Type.InternalType(),
		)
	...
	t.Run(
		"NestedTypeDetails",
		func(t *testing.T) {
			intInfo, err := NewTypeInfo(
				TYPE_INTEGER,
			)
			require.NoError(t, err)
			strInfo, err := NewTypeInfo(
				TYPE_VARCHAR,
			)
			require.NoError(t, err)
...
func TestTypeInfoInternalType(t *testing.T) {
	tests := []struct {
		...
		{
			"Boolean",
			func() (TypeInfo, error) { return NewTypeInfo(TYPE_BOOLEAN) },
			TYPE_BOOLEAN,
		},
		{
			"Integer",
			func() (TypeInfo, error) { return NewTypeInfo(TYPE_INTEGER) },
			TYPE_INTEGER,
		},
		{
			"Decimal",
			func() (TypeInfo, error) { return NewDecimalInfo(10, 2) },
			TYPE_DECIMAL,
		},
		{
			"Enum",
			func() (TypeInfo, error) { return NewEnumInfo("a", "b") },
			TYPE_ENUM,
		},
		...
		{"Map", func() (TypeInfo, error) {
			keyInfo, _ := NewTypeInfo(
				TYPE_VARCHAR,
			)
			valueInfo, _ := NewTypeInfo(
				TYPE_INTEGER,
			)
