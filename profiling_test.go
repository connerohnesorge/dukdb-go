...
func TestProfilingInfo_Structure(t *testing.T) {
	...
	assert.Equal(t, "100ms", info.Metrics["TOTAL_TIME"])
	assert.Equal(t, "42", info.Metrics["ROWS_RETURNED"])
	require.Len(t, info.Children, 1)
	assert.Equal(t, "SCAN", info.Children[0].Metrics["OPERATOR_TYPE"])
}

func TestProfilingContext_NewWithNilClock(t *testing.T) {
...
func TestProfilingContext_WithClock(t *testing.T) {
...
func TestProfilingContext_EnableDisable(t *testing.T) {
	ctx := NewProfilingContext(nil)
...
func TestProfilingContext_StartAndElapsed(t *testing.T) {
	mClock := quartz.NewMock(t)
	ctx := NewProfilingContext(mClock)

	// Without enabling, Start should be no-op
	ctx.Start()
	assert.Equal(t, time.Duration(0), ctx.Elapsed())
...
func TestProfilingContext_AddOperator(t *testing.T) {
	mClock := quartz.NewMock(t)
	ctx := NewProfilingContext(mClock)
	ctx.Enable()
	ctx.Start()
	...
	// Add an operator
	ctx.AddOperator("SCAN", 100, 50*time.Millisecond)
	...
	require.Len(t, info.Children, 1)
	assert.Equal(t, "SCAN", info.Children[0].Metrics["OPERATOR_TYPE"])
	assert.Equal(t, "100", info.Children[0].Metrics["ROWS"])
}
...
func TestProfilingContext_Complete(t *testing.T) {
	...
	assert.Equal(t, "200ms", info.Metrics["TOTAL_TIME"])
	assert.Equal(t, "100", info.Metrics["ROWS_RETURNED"])
}

func TestProfilingContext_GetInfo_NotEnabled(t *testing.T) {
	ctx := NewProfilingContext(nil)
...
func TestProfilingContext_GetInfo_Empty(t *testing.T) {
	ctx := NewProfilingContext(nil)
	ctx.Enable()
...
func TestProfilingContext_GetInfo_ClearsAfterRetrieval(t *testing.T) {
	mClock := quartz.NewMock(t)
	ctx := NewProfilingContext(mClock)
	ctx.Enable()
	ctx.Start()
...
func TestOperatorMetrics_Timing(t *testing.T) {
	mClock := quartz.NewMock(t)
	metrics := newOperatorMetrics(mClock, "FILTER")

	assert.Equal(t, "FILTER", metrics.OperatorType())
	...
	assert.Equal(t, 75*time.Millisecond, metrics.Duration())
...
func TestOperatorMetrics_RowCount(t *testing.T) {
	mClock := quartz.NewMock(t)
	metrics := newOperatorMetrics(mClock, "PROJECTION")
...
func TestGetProfilingInfo_NotEnabled(t *testing.T) {
...
func TestGetProfilingInfo_WithProfiling(t *testing.T) {
	setupProfilingTest(t)
	db, err := sql.Open("dukdb", ":memory:")
	require.NoError(t, err)
	defer db.Close()
	...
	assert.Equal(t, "50", info.Metrics["ROWS_RETURNED"])
}

// Deterministic timing test using mock clock
func TestProfilingContext_DeterministicTiming(t *testing.T) {
	mClock := quartz.NewMock(t)
	...
	ctx.AddOperator("SCAN", 1000, 100*time.Millisecond)
	ctx.AddOperator("FILTER", 500, 50*time.Millisecond)
	ctx.AddOperator("PROJECTION", 500, 25*time.Millisecond)
	...
	assert.Equal(t, "500", info.Metrics["ROWS_RETURNED"])

	// Verify operators
	require.Len(t, info.Children, 3)
	assert.Equal(t, "SCAN", info.Children[0].Metrics["OPERATOR_TYPE"])
	assert.Equal(t, "100ms", info.Children[0].Metrics["OPERATOR_TIMING"])
	assert.Equal(t, "FILTER", info.Children[1].Metrics["OPERATOR_TYPE"])
	assert.Equal(t, "50ms", info.Children[1].Metrics["OPERATOR_TIMING"])
	assert.Equal(t, "PROJECTION", info.Children[2].Metrics["OPERATOR_TYPE"])
	assert.Equal(t, "25ms", info.Children[2].Metrics["OPERATOR_TIMING"])
}
